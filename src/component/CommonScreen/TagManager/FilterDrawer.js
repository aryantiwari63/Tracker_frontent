        ...
        >
          <div className="flex justify-between px-[1rem] py-[0.5rem]">
            ...
            {isOpen === mainFilter.id ? (
              <img
                className="w-3 h-4"
                src={`/assets/images/dashboardactivedropdown.svg`}
                alt="toggle"
              />
            ) : (
              <img
  ...
  const renderIncludedFilters = (mainFilter) => (
    <>
      ...
      {mainFilter.value === "created_by" &&(
        <div className=" mt-2 px-[1rem] form-group has-search show-right-border">
          ...
          <input
            ...
            onChange={(e) => {
              if (e.keyCode == 13) {
                ...
                console.log(mainFilter?.searchedValue,"platformFilters>>>>>>>>",mainFilter?.searchedValue.filter((item) =>
                item.title
                  .toLowerCase()
      ...
      <div
        className={`mt-1 ${
          mainFilter.value === "created_by"
            ? "h-[15rem] overflow-y-auto shadow-md rounded bg-white "
            : "shadow-md rounded"
        }`}
      >
        {mainFilter?.includedFilter && mainFilter?.includedFilter.length > 0 ? (
          mainFilter?.includedFilter.map((subFilter, i) => (
            <div
              ...
              className={`flex custom-checkbox dashboard-custom-select items-center py-[0.8rem] border-b border-[#F2F2F2] rounded px-[1rem] cursor-pointer ${
      ...
      <div key={key} className="flex flex-wrap gap-1">
        {filters[key].map((filter, index) => (
          <div
            ...
            className={`flex border-[#91d5ff] border px-2 py-0.5 my-1 bg-white text-[${color}] text-[12px] items-center`}
          >
            <div style={{ color }}>
              {filter.parentTitle} :{" "}
