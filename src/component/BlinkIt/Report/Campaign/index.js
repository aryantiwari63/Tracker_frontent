...
import {
  BLINKIT_CAMPAIGN_REPORT,
  BLINKIT_CAMPAIGN_REPORT_DOWNLOAD,
  LIMIT,
  headersDataBlinkit,
...
const CampaignReport = () => {
  ...
  const [showHeader, setShowHeader] = React.useState([...headersDataBlinkit]);
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(BLINKIT_CAMPAIGN_REPORT, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      const res = await _POST(BLINKIT_CAMPAIGN_REPORT_DOWNLOAD, downloadFilter,{responseType: "arraybuffer",
      headers: {"Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"}});
    const blob = new Blob(["\ufeff", res.data], { type: 'text/csv;charset=utf-8' });
      const pom = document.createElement('a');
      pom.href = URL.createObjectURL(blob);
      pom.setAttribute('download', `Campaign report_${downloadFilter.start_date}-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`);
      pom.click();
