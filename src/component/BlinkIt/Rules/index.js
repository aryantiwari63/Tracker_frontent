...
import { blinkitRuleHeaders, PERMISSIONS } from "../../../utils/constants";
...
const Rules = () => {
  ...
  // eslint-disable-next-line no-unused-vars
  const [showHeader, setShowHeader] = React.useState([...blinkitRuleHeaders]);
  ...
  useEffect(() => {
    ...
    getRulesApi();
  }, [showPopup, status, campaignType]);
  //  [showPopup, status, campaignType, selectedAccount]);
  useEffect(() => {
    if (recallRules == true) {
      getRulesApi();
    }
  ...
  //     // setSelectedAccount(filterAccounts.map((account) => account.label));
  //     setSelectedAccount(
  //       accounts.length === filterAccounts.length
  //         ? ["all"]
  //         : filterAccounts.map((account) => account.label)
  //     );

  //     setDefaultCheckAccount(filterAccounts);
  //   } catch (error) {
  //     console.error(error);
  //   }
  // };
  ...
  //       let brandObj = _.find(accounts, { value: val });
  //       return brandObj ? brandObj.label : undefined;
  //     });

  //     setSelectedAccount(accounts.length === labels.length ? ["all"] : labels);
    ...
    <>
      ...
      <section className="mt-4 flex-nowrap pt-4 pl-2 bg-white">
        <div className="flex">
          ...
          <div className="relative text-gray-600 ml-2">
            <input
              type="search"
              value={search}
              onChange={(e) => {
              ...
              className="focus:border-[#13a976] focus:ring-[#13a976] rounded-md sm:text-sm focus:ring-1  w-96 border border-[#ccc] bg-white h-8 px-5 pr-10  text-sm focus:outline-none "
            />
            <button
      ...
      <section>
        <div className="rules__card ">
          <WhenPermitted permission={PERMISSIONS.CREATE_RULE} platform="blinkit">
          <button
            className="card__btn row mb-4 blinkit_btn"
