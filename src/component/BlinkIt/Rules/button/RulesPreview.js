...
import "./style.css";
import { campaignBlinkitHeaders, blinkitKeywordHeaders, blinkitCategoryHeaders } from "../../../../utils/rulesConstants"
...
const RulesPreview = ({ ruleId, accountsData, setShowPopup, showPopup, getRulesApi }) => {
    ...
    useEffect(() => {
        ...
        if (ruleId.entity == 'Campaign') {
            setHeaders(campaignBlinkitHeaders);
        }
        if (ruleId.entity == 'Keyword') {
            setHeaders(blinkitKeywordHeaders);
        }
        if (ruleId.entity == 'Category') {
            setHeaders(blinkitCategoryHeaders);
        }
    ...
    function getCustomButton() {
        ...
        if (hasPermission) {
            customButton.push( {
                ...
                label: "Save Changes",
                style: `${disable ? "text-white font-semibold py-2 px-4  opacity-50 cursor-not-allowed" : "text-white"} bg-[#11b07a] border rounded`,
                disabled: disable 
            },);
        } 
        return customButton;   
        ...
        > <><div style={{ maxHeight: 'inherit' }} className="relative overflow-x-auto shadow-md sm:rounded-lg overflow-scroll">
            <div><table className=" w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" className="p-[1.2rem]" style={{ minWidth: '40px' }}>
                            <div className="flex items-center">
                                <input checked={resultData?.length == selected?.length && resultData?.length > 0} onChange={(e) => { handleAllCheckBox(e) }} id="checkbox-all" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600  accent-green-600" disabled={!hasPermission} />
                        ...
                        resultData && resultData?.length > 0 ? 
                         resultData?.map((item, index) => (<tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td className="w-4 p-4">
                                <div className="flex items-center">
                                    <input onChange={(e) => { handleCheckboxChange(e.target.checked, item.id) }} checked={selected?.includes(item?.id)} id="checkbox-table-1" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 accent-green-600" disabled={!hasPermission} />
                                ...
                                if (item1.value == 'platform') {
                                    return (
                                        <td key={index1} className="px-6 py-4">
                                            {item.platform ? item.platform : "Blinkit"}
                                        </td>
                                    )
                                }
                                else if (item1.value == 'platform_id') {
                                    return (
                                        <td key={index1} className="px-6 py-4">
