...
import { getBlinkitCategoryListByBrand } from "../../../../../../../redux/action-creator/blinkit/createCampaignAction";

const CategoryKeywordBrandSuggestion = ({
  setCampaignData,
  campaignData,
  inputError,
  setInputError,
  ...
  React.useEffect(() => {
    dispatch(
      getBlinkitCategoryListByBrand({
        brand_id: campaignData?.brandDetailsData?.map((data) => data.brand_id),
      })
    );
  }, []);
  const { categoryListByBrand } = useSelector(
  ...
  React.useEffect(() => {
    ...
    // console.log("debugerrr inputError", inputError);
  }, [inputError]);
    ...
    <>
      <table className="w-full">
        ...
        <tbody className="targetKeyword__table ">
          {categoryListByBrand && categoryListByBrand.length
            ? categoryListByBrand.map((data) => {
                return (
                  <>
...
export default CategoryKeywordBrandSuggestion;
