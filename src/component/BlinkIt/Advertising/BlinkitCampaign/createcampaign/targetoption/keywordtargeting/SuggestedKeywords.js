...
const SuggestedKeywords = ({
  ...
  React.useEffect(() => {
    dispatch(
      getBlinkitKeywordList({
        data: campaignData?.products?.map((data) => data?.id),
        type: "new",
      })
    );
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (brandedKeyword && !genericKeyword) {
      data = keywordList.filter((key) => key.is_brand_keyword === true);
    } else if (genericKeyword && !brandedKeyword) {
      data = keywordList.filter((key) => key.is_brand_keyword === false);
