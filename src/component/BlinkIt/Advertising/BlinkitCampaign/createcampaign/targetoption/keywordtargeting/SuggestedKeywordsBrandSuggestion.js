...
import { getBlinkitKeywordListByBrandsProducts } from "../../../../../../../redux/action-creator/blinkit/createCampaignAction";

const SuggestedKeywordsBrandSuggestion = ({
  setSelectedKeywords,
  campaignData,
  setKeywordData,
  keywordData,
  ...
  React.useEffect(() => {
    dispatch(
      getBlinkitKeywordListByBrandsProducts({
        data: campaignData?.brandDetailsData,
        type: "new",
      })
    );
  }, []);
  const { keywordListbybrandproducts } = useSelector(
    (state) => state?.BlinkitCreateCampaignReducer
  );
  React.useEffect(() => {
    setKeywordData([...keywordListbybrandproducts]);
  ...
  React.useEffect(() => {
    ...
    if (brandedKeyword && !genericKeyword) {
      data = keywordListbybrandproducts.filter(
      ...
      data = [...keywordListbybrandproducts];
    }
  ...
  return (
    <>
      <div className="row p-5 bg-[#F7FFF9] rounded-2xl">
        ...
        <div className="row pt-4 max-h-[500px] overflow-y-auto">
          {keywordData &&
            keywordData.length &&
...
export default SuggestedKeywordsBrandSuggestion;
