...
const TargetKeywordContent = ({
  ...
  const handleDelete = (data) => {
    ...
    // });
    if (campaignData && campaignData.keywords?.length) {
      let tempData = [...campaignData.keywords];
      // eslint-disable-next-line no-console
      // console.log("campaignData tempData 1", tempData, data);
      let newData = tempData.filter((item) => item.id !== data.id);
      setNewKeywordData(newData);
      // eslint-disable-next-line no-console
      // console.log("campaignData tempData 2", newData, data);
    }
