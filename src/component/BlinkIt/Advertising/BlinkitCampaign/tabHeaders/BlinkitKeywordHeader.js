...
const [loading, setLoading]=useState(false)
  ...
  const actionText = (text) => {
    ...
    switch (text) {
      case "enable":
        actionPhrase = `(${campaignCount} of ${totalKeywords} keyword(s) will be added to the campaign on BlinkIt platform`;
        // setSelectedAction("enable");
        break;
      case "pause":
        actionPhrase = `(${campaignCount} of ${totalKeywords} keyword(s) will be removed from the campaign on BlinkIt platform`;
  ...
  const handlePayload = () => {
    ...
    const data = selectedCheckBox.keyword?.map((data) => {
      if (selectedAction === "set_bid") {
        message = `Set keyword bid from ${data.cpm_bid} to ₹${bid} `;
      }
    ...
    try {
      ...
      let rowData = selectedCheckBox[tabName].map((item) => {
        let obj = {
          ...
          entity_type: item.campaign_type,
          set_bid: item.cpm_bid > item.min_bid ? item.cpm_bid : item.min_bid,
          // history_data: {
          //   previous_bid: parseFloat(
          //     item.cpm_bid
          //   ),
      ...
      data.action = selectedAction == "pause" ? "pause_keyword" : "enable_keyword"
      data.action_type = "keyword"
      data.user_name =  localStorage.getItem('name')
      data.action_message = selectedAction == "pause" ? "Bulk pause keyword" : "Bulk enable keyword"
...
export default BlinkitKeywordHeader;
