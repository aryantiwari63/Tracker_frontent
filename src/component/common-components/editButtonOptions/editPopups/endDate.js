...
import { RPA_ACTION_EDIT, FLIPKART_ADGROUP_ABORT_LIST } from "../../../../utils/constants";
...
export const EndDate = ({ setOpenState }) => {
  ...
  const handleApplyButton = () => {
    const data = selectedCheckBox?.campaign?.map((data) => {
      return {
        ...
        action_status: 1,
        action_message: `Change the end date of the campaign to ${selectedDate.split("T").join(" ")}`,
  ...
  const getAdGroupList = async () => {
    const campaign_ids = selectedCheckBox?.campaign?.map((item) => item.campaign_id)
    const res = await _POST(FLIPKART_ADGROUP_ABORT_LIST, { campaign_ids })
    ...
    <>
      ...
      >
        <div className="">
          ...
          {adGroup.length > 0 && <div className="p-4"> <p className="text-red-500">*Following campaigns contains ad groups with aborted status please unselect to continue</p>
