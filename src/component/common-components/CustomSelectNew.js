...
import { ringThemeObj, selectOptionThemeObj } from "../../style/StyleConstants";
    ...
    <div className="relative" ref={outerRef}>
      ...
      {showOptions && (
        <div className=" absolute top-full pt-0 left-0 w-full z-[99] ">
          <ul className="border w-full bg-white text-gray-600 text-xs shadow-[0_5px_5px_#ddd] overflow-y-auto max-h-[250px]">
            {listArr?.map((item, i) => (
              <li
                key={i}
                onClick={() => {
