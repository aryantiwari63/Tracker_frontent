...
function SomCategoryNodeFilterComponent({ expanded, handleToggle, handleSelectAll,
    ...
    const handleUpdate = (option, isChecked) => {
        ...
        if (isChecked) {
            newSomCategories = selectedFiltersWidget.selectCategory_node.filter(
                (c) => c.value !== option.value
            );
        } else {
            newSomCategories = [
                ...(selectedFiltersWidget.selectCategory_node || []),
                option,
            ];
        }
        getDistinctFiltersSomFn('category_node',newSomCategories);
    };


    const handleSelectAllUpdate = (options, allSelected) => {
  const current = selectedFiltersWidget.selectCategory_node || [];
    console.log('currentcurrent',current)
  let newItems = [];

  if (allSelected) {
    // Unselect visible items only
    newItems = current.filter((b) => !options.some((opt) => opt.value === b.value));
  } else {
    // Add visible items, avoiding duplicates
    const uniqueItems = [
      ...current,
      ...options.filter(
        (opt) => !current.some((sel) => sel.value === opt.value)
      ),
    ];
    newItems = uniqueItems;
  }

  getDistinctFiltersSomFn('category_node',newItems);
};

    
    const sortedCategories = useMemo(() => {
        const list = Array.isArray(filters?.category_node) ? filters.category_node : [];
        return searchAndRank(list, {
            ...
            localMode: "OR",
            globalMode: "OR",
            selectedValues: selectedFiltersWidget.selectCategory_node?.map(c => c.value) || [],
            labelKey: "label",
        });
    }, [searchTerm, categorySearch, filters?.category_node]);

    // ✅ Determine "Select All" checked state based on visible filtered items
    const allVisibleSelected =
  sortedCategories.length > 0 &&
  sortedCategories.every((opt) =>
    isOptionChecked("category_node", opt, selectedFiltersWidget, kpi)
  );
            ...
            <div className="flex flex-col px-8 p-4 space-y-8">
                ...
                <div className="border-b border-gray-100 pb-6 last:border-0">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="font-semibold text-gray-900 text-base">Category Node</h3>
                        {sortedCategories?.length > 3 && (
                            <button
                                type="button"
                                className="text-gray-500 hover:text-gray-700"
                                onClick={() => handleToggle("category_node")}
                            >
                                {expanded["category_node"] ? <IoIosArrowUp size={20} /> : <IoIosArrowDown size={20} />}
                            </button>
                        )}
                    </div>
                    ...
                    {sortedCategories?.length > 0 && (
                        <div className="space-y-2">
                            <div className={`space-y-2 transition-all duration-300 ${expanded["category_node"] ? "max-h-60 overflow-y-auto pr-2" : "max-h-[102px] overflow-hidden"}`} >

                                <label className="flex items-center font-bold gap-2 text-sm text-gray-700 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        className="w-4 h-4 text-blue-600 rounded"
                                        // checked={isOptionChecked("category_node", filters?.category, selectedFilters?.selectCategory_node)}
                                        // onChange={() => {
                                        //     const allSelected = filters?.category.every((opt) =>
                                        //         isOptionChecked("category_node", opt, selectedFilters)
                                        //     );
                                        //     handleSelectAllUpdate(filters?.category, allSelected);
                                        //     handleSelectAll("category_node", filters?.category);
                                        // }}
                                        ...
                                        onChange={() => {
                                            ...
                                            handleSelectAll("category_node", sortedCategories);
                                        }}
                                    />
                                    {selectAllLabel}
                                </label>
                                ...
                                {sortedCategories?.map((option, i) => {
                                    ...
                                    const isChecked = isOptionChecked("category_node", option, selectedFiltersWidget, kpi);
                                    return (
                                                ...
                                                <span>
                                                    <input
                                                        ...
                                                        onChange={() => {
                                                            ...
                                                            handleCheck("category_node", option.value, isChecked); // keep drawer state in sync
                                                        }}
                                                    />
                                                </span>
                                                {option.label}
                    ...
                    {sortedCategories?.length > 2 && (
                        <button
                            ...
                            onClick={() => handleToggle("category_node")}
                        >
                            {expanded["category_node"] ? "View Less" : "View More"}
                            <span className="ml-1">
                                {expanded["category_node"] ? <IoIosArrowUp size={16} /> : <IoIosArrowDown size={16} />}
                            </span>
                        </button>
                    )}
                </div>
...
export default SomCategoryNodeFilterComponent;