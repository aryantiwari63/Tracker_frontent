...
const CategoryFilterComponent = ({ expanded, handleToggle, searchTerm }) => {
    ...
    const handleApply = async () => {
        ...
        if (activeClientProject?.isFilterDateWise) {
            ...
            combineFilterWidget = await getCombineFilterWidget("OSA", (selectedFiltersWidget.selectedPlatform?.map(i => i.value) ?? []), (bid ?? []), (cid ?? []), (selectedFiltersWidget.selectedMotherPack?.map(i => i.value) ?? []), (selectedFiltersWidget.selectedProductId?.map(i => i.value) ?? []), selectedMsl, (tagSkuList ?? []), dateRangeData);
        } else {
            combineFilterWidget = await getCombineFilterWidget("OSA", (selectedFiltersWidget.selectedPlatform?.map(i => i.value) ?? []), (bid ?? []), (cid ?? []), (selectedFiltersWidget.selectedMotherPack?.map(i => i.value) ?? []), (selectedFiltersWidget.selectedProductId?.map(i => i.value) ?? []), selectedMsl, (tagSkuList ?? []));
        }
        ...
        setFilters(prevFilters => ({
            ...
            category: (kpi === "SOS" || kpi === "OR") ? combineFilterWidgetKW?.categories ?? [] : combineFilterWidget?.categories ?? [],
    ...
    }, [selectedFiltersWidget?.selectedCategory, filteredData]);
