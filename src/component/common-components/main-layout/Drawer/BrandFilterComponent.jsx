...
function BrandCommonFilterComponent({ expanded, handleToggle, handleSelectAll, handleCheck, searchTerm }) {
    const {
        kpi,
    ...
    const handleApplyClick = async () => {
        ...
        if (activeClientProject?.brandTreeSelect) {
            if (kpi === "SOS" || kpi === "OR") {
                updateSelectedKeywordBrandV2(current);
            } else {
                updateSelectedBrandV2(current);
            }
        } else {
            if (kpi == "GLOBALVIEW") {
                const bid = [...new Set(current?.flatMap(i => i.value) ?? [])];
                let combineFilterWidget = await fetchGlobalViewCombineFiltersPdpKw("GLOBALVIEW", (selectedFiltersWidget.selectedPlatform?.map(i => i.value) ?? []), (bid ?? []), (selectedFiltersWidget.selectedCategory?.flatMap(i => i.id_in_db) ?? []));
                ...
                if ([2, 101, 102, 103].indexOf(activeClientProject?.client_project_id) > -1 || activeClientProject?.useCombineFilter) {
                    ...
                    const bid = [...new Set(current?.flatMap(i => i.id_in_db) ?? [])];
                    ...
                    if (activeClientProject?.isFilterDateWise) {
                        ...
                        combineFilterWidget = await getCombineFilterWidget("OSA", (selectedFiltersWidget.selectedPlatform?.map(i => i.value) ?? []), (bid ?? []), (selectedFiltersWidget.selectedCategory?.flatMap(i => i.id_in_db) ?? []), (selectedFiltersWidget.selectedMotherPack?.map(i => i.value) ?? []), (selectedFiltersWidget.selectedProductId?.map(i => i.value) ?? []), selectedMsl, (tagSkuList ?? []), dateRangeData);
                    } else {
                    ...
                    updateSelectedBrand(current);
                }
            }
        }

        if (typeof handleCheck === "function") {
            const allBrands = Array.isArray(filters?.brand) ? filters.brand : [];
            allBrands.forEach((brand) => {
                const isChecked = current.some((b) => b.value === brand.value);
                handleCheck("brand", brand.value, !isChecked);
    ...
    const sortedBrands = useMemo(() => {
        ...
        return sortedList;
    }, [filters?.brand, searchTerm, brandSearch, localSelectedBrands]);
