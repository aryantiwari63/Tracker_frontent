...
function TagManagerFilterComponent({ expanded, handleToggle, handleSelectAll,
    ...
    const handleUpdate = (option, isChecked) => {
        ...
        if (isChecked) {
            newProducts = selectedFiltersWidget?.selectedTags.filter(
                (p) => p.id !== option.id
            );
        } else {
            newProducts = [...(selectedFiltersWidget?.selectedTags || []), option];
        }
    ...
    const handleSelectAllUpdate = (options, allSelected) => {
        ...
        if (allSelected) {
            ...
            newItems = selectedFiltersWidget.selectedTags.filter(
            ...
            const uniqueItems = [
                ...
                ...options.filter(
                    (opt) =>
                        !selectedFiltersWidget.selectedTags.some(
                            (sel) => sel.id === opt.id
                        )
                ),
            ];
    ...
    const sortedMotherPack = useMemo(() => {
        let list = Array.isArray(tagList) ? tagList : [];
            list = list.filter(item => item?.tag_type === "Product");
        ...
        return [...filteredList].sort((a, b) => {
            const aChecked = selectedFiltersWidget?.selectedTags?.some((sel) => sel.tag_name === a.tag_name);
            const bChecked = selectedFiltersWidget?.selectedTags?.some((sel) => sel.tag_name === b.tag_name);
    ...
    const allVisibleSelected =
        ...
        sortedMotherPack.every((opt) =>
            isOptionChecked("tags", opt, selectedFiltersWidget)
        );
                        ...
                        <div className="space-y-2">
                            <div className={`space-y-2 transition-all duration-300 ${expanded["tags"] ? "max-h-60 overflow-y-auto pr-2" : "max-h-[102px] overflow-hidden"}`} >
                                ...
                                <label className="flex items-center font-bold gap-2 text-sm text-gray-700 cursor-pointer">
                                    <input
                                        ...
                                        onChange={() => {
                                            ...
                                            handleSelectAll("tags", sortedMotherPack);
                                        }}
                                    />
                                    Select All
                                </label>
                                ...
                                {sortedMotherPack?.map((option, i) => {
                                    ...
                                    const isChecked = isOptionChecked("tags", option, selectedFiltersWidget);
                                    return (
                                                ...
                                                <span>
                                                    <input
                                                        ...
                                                        onChange={() => {
                                                            ...
                                                            handleCheck("tags", option.id, isChecked); // keep drawer state in sync
                                                        }}
                                                    />
                                                </span>
