...
const PreviewBtn = ({ rowData, source }) => {
  ...
  const updateEmails = async () => {
    try {
      ...
      let emailChecker = validEmail.every(
        (v) => v === true || v === undefined || v === ""
      );
      if (email.length > 0 && emailChecker) {
        ...
        if (response?.status === 200) {
          ...
          dispatch(
            setToastMessageHandler("Failed to update rule status", false)
          );
        }
        setShowPopup(false);
      }
  ...
  return (
    <>
      <div className="row justify-center">
        <div className="row justify-center">
          <button
            className={`preview__btn ${source == 'blinkit'  ? "" :  "preview__btn_zepto"} w-14`}
          ...
          <WhenPermitted permission={PERMISSIONS.CREATE_RULE} platform={source}>
          <button className={`preview__delbtn ml-1  ${source == 'blinkit' ? "preview__btn" : source == 'zepto' ? "preview__btn_zepto":""}`} onClick={() => setOpen(!open)}>
            ...
            cutomButton={hasPermission && [
              {
                ...
                label: "Save Changes",
                style: `${source == 'blinkit' ? "blinkit_btn" :"preview__apply_zepto" }`,
              },
            ]}
          >
            <p>
