...
import "./style.css";
import { campaignHeaders, adgroupHeaders, keywordHeaders, productHeaders, categoryHeaders } from "../../../../utils/rulesConstants"
    ...
    function getCustomButton() {
        ...
        if (hasPermission) {
            customButton.push({
                ...
                label: "Save Changes",
                style: `${disable ? " text-white font-semibold py-2 px-4 opacity-50 cursor-not-allowed" : "text-white"} border bg-[#5a0c82] rounded`,
                disabled: disable 
            });
        } 
        return customButton;   
                    ...
                    <tr>
                        <th scope="col" className="p-[1.1rem]" style={{ minWidth: '40px' }}>
                            <div className="flex items-center">
                                <input checked={resultData?.length == selected?.length && resultData?.length > 0} onChange={(e) => { handleAllCheckBox(e) }} id="checkbox-all" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-800 dark:focus:ring-purple-900 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 accent-purple-900" disabled={!hasPermission} />
                        ...
                        resultData && resultData?.length > 0 ? 
                         resultData.map((item, index) => (<tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td className="w-4 p-4">
                                <div className="flex items-center">
                                    <input onChange={(e) => { handleCheckboxChange(e.target.checked, item.id) }} checked={selected?.includes(item?.id)} id="checkbox-table-1" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-800 dark:focus:ring-purple-900 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 accent-purple-900" disabled={!hasPermission} />
                            ...
                            {headers && headers.map((item1, index1) => {
                                ...
                                if (item1.value == 'platform') {
                                    return (
                                        <td key={index1} className="px-6 py-4">
                                            {item.platform ? item.platform : "Zepto"}
                                        </td>
                                    )
                                }
                                else {
