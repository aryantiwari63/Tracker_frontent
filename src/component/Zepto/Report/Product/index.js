...
import {
  ...
  // categoryDataBlinkit,
  // revenueHeaders,
  ZEPTO_PRODUCT_REPORT,
  productDataZepto,
  ZEPTO_PRODUCT_REPORT_DOWNLOAD,
  // ZEPTO_ACCOUNT,
...
const ProductReport = () => {
  ...
  const [showHeader, setShowHeader] = React.useState([...productDataZepto]);
  ...
  function onChangeDate(item) {
    ...
    if (!calState.fullCalender) {
      ...
      productReportApi([item.selection]);
    }
  }
  const applyDate = () => {
  ...
  const productReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(ZEPTO_PRODUCT_REPORT, post, {
  ...
  const categoryReportDownloadApi = async () => {
    try {
      ...
      const res = await _POST(ZEPTO_PRODUCT_REPORT_DOWNLOAD, downloadFilter,{responseType: "arraybuffer",
      headers: {"Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"}});
    const blob = new Blob(["\ufeff", res.data], { type: 'text/csv;charset=utf-8' });
      const pom = document.createElement('a');
      pom.href = URL.createObjectURL(blob);
      pom.setAttribute('download', `Product report_${downloadFilter.platform}_${downloadFilter.start_date}-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`);
      pom.click();
