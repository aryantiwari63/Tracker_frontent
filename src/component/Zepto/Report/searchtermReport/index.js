...
import {
  LIMIT,
  SEARCH_REPORT_API_URL,
  SEARCH_REPORT_DOWNLOAD_API_URL,
...
const SearchReport = () => {
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(SEARCH_REPORT_API_URL, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      setDownloadLoading(true);
      const res = await _POST(SEARCH_REPORT_DOWNLOAD_API_URL, downloadFilter);
