...
// import BlinkitTable from "../../table/BlinkitTable";
...
// import {
//   CAMPAIGN_REPORT_API_URL,
//   CAMPAIGN_REPORT_DOWNLOAD_API_URL,
//   LIMIT,
//   downloadheadersData,
//   headersData,
...
//   const [showHeader, setShowHeader] = React.useState([...headersData]);
...
//   const campaignReportApi = async () => {
//     try {
//       setLoading(true);

//       const res = await _POST(CAMPAIGN_REPORT_API_URL, post);
...
//       const res = await _POST(CAMPAIGN_REPORT_DOWNLOAD_API_URL, downloadFilter);
...
import {
  CREATIVE_REPORT_API_URL,
  CREATIVE_REPORT_DOWNLOAD_API_URL,
  LIMIT,
  creativeHeaders,
  downloadcreativeHeaders,
} from "../../../../utils/constants";
...
const CreativeReport = () => {
  ...
  const [selectedPlatformVal, setSelectedPlatformVal] = React.useState([]);
  const [selectedTypeVal, setSelectedTypeVal] = React.useState([]);
  const [showHeader, setShowHeader] = React.useState([...creativeHeaders]);
  // eslint-disable-next-line no-unused-vars
  const [downloadHeader, setDownloadHeader] = React.useState([
    ...downloadcreativeHeaders,
  ]);
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(CREATIVE_REPORT_API_URL, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      setDownloadLoading(true);
      const res = await _POST(CREATIVE_REPORT_DOWNLOAD_API_URL, downloadFilter);
