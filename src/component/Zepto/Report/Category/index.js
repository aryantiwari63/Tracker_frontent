...
import {
  ...
  LIMIT,
  // BLINKIT_CATEGORY_REPORT,
  // BLINKIT_CATEGORY_REPORT_DOWNLOAD,
  // REVENUE_REPORT_API_URL,
  // REVENUE_REPORT_DOWNLOAD_API_URL,
  // downloadkeywordHeaders,
  // categoryDataBlinkit,
  // revenueHeaders,
  ZEPTO_CATEGORY_REPORT,
  categoryDataZepto,
  ZEPTO_CATEGORY_REPORT_DOWNLOAD,
  GET_ZEPTO_ACCOUNTS,
  // ZEPTO_ACCOUNT,
...
const CategoryReport = () => {
  const dateFilters = defaultDateRange(); //zepto
  ...
  const [showHeader, setShowHeader] = React.useState([...categoryDataZepto]);
  ...
  // eslint-disable-next-line no-unused-vars
  const [zeptoAccountData,setZeptoAccountData] = useState([]) 
  ...
  const applyFilter = React.useCallback(() => {
    ...
    // post.start_date =  convertDate(dateRange[0].startDate),
    // post.end_date = convertDate(dateRange[0].endDate),
    
    campaignReportApi(true);
  }, [showHeader,selectedPlatformVal]);

  const apply = () => {
    // categoryReportDownloadApi();
  ...
  const fetchZeptoAccount = async()=>{
    const result = await _GET(GET_ZEPTO_ACCOUNTS);
      const data = result.data.data;
      const tempAccount = data.map((item) => ({
        label: item.account_name,
        value: item.id,
      }));
      setZeptoAccountData(tempAccount)
  }
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(ZEPTO_CATEGORY_REPORT, post, {
  ...
  const categoryReportDownloadApi = async () => {
    try {
      ...
      if (downloadFilter?.date === "Cumulative") {
        headersDownload = headersDownload.filter(
          (item) => item.key !== "campaign_name" && item.key !== "category_name"
        );
      }
      setDownloadHeader(headersDownload);
      const res = await _POST(ZEPTO_CATEGORY_REPORT_DOWNLOAD, downloadFilter,{responseType: "arraybuffer",
      headers: {"Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"}});
    const blob = new Blob(["\ufeff", res.data], { type: 'text/csv;charset=utf-8' });
      const pom = document.createElement('a');
      pom.href = URL.createObjectURL(blob);
      pom.setAttribute('download', `Category report_${downloadFilter.platform}_${downloadFilter.start_date}-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`);
      pom.click();
