...
// import campaignreportdata from "../../data/campaignreportdata.json"
...
//       const res = await _POST(KEYWORD_REPORT_DOWNLOAD_API_URL, downloadFilter);
...
import { convertDate } from "../../../../utils/helpers";
import {
  ADGROUP_REPORT_API_URL,
  ADGROUP_REPORT_DOWNLOAD_API_URL,
  LIMIT,
  adGroupHeaders,
  downloadadGroupHeaders,
} from "../../../../utils/constants";
import { _POST } from "../../../../services/axios.method";
import TableTitle from "../../../common-components/flipkarttable/TableTitle";
...
const AdGroupReport = () => {
  ...
  const [selectedPlatformVal, setSelectedPlatformVal] = React.useState([]);
  const [selectedTypeVal, setSelectedTypeVal] = React.useState([]);
  const [showHeader, setShowHeader] = React.useState([...adGroupHeaders]);
  // eslint-disable-next-line no-unused-vars
  const [downloadHeader, setDownloadHeader] = React.useState([
    ...downloadadGroupHeaders,
  ]);
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(ADGROUP_REPORT_API_URL, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      setDownloadLoading(true);
      const res = await _POST(ADGROUP_REPORT_DOWNLOAD_API_URL, downloadFilter);
