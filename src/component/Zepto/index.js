...
import ZeptoCampaignTable from "./table/ZeptoCampaignTable";
import { getZeptoPerformanceBreakDown } from "../../redux/action-creator/zepto/dashboardAction";
import _ from "lodash";
class ZeptoDashBoard extends React.Component {
  constructor(props) {
    super(props);
  ...
  async fetchTags() {
    try {
      ...
      const response = await _GET(
        `${GET_ALL_TAGS}?platform=zepto&data_level=campaign&accounts=${acc}`
      );
  ...
  async initLoad() {
    try {
      let filters = this.state.filters;
      let dashData = await dashboardZeptoOverview(filters);
      const res = await dashboardzeptoGraph({
        ...this.state.filters,
        filters: this.state.graphFilters,
      });
      this.props.dispatch(
        getZeptoPerformanceBreakDown({
          ...filters,
          filters: this.state.graphFilters,
        })
      );
  ...
  applyFilters(name, value) {
    ...
    if (name === "brand") {
      ...
      // let orderAccounts = this.state.options.brand.filter(ele => value.includes(ele.label));
...
export default connect(null, mapDispatchToProps)(ZeptoDashBoard);
