...
const CategoryActionHeader = ({ campaignSelection, setShowBulkPopup }) => {
  ...
  let categoryCount = selectedCheckBox?.category?.length;
  const selectionText = `${categoryCount} category(s) selected: `;
  ...
  const handleApplyButton = () => {
    ...
    const data = selectedCheckBox?.category?.map((data) => {
      if (selectedAction === "pause" || selectedAction === "enable") {
        message = `${selectedAction} category`;
      }
  ...
  const actionText = (text) => {
    ...
    switch (text) {
      case "enable":
        actionPhrase = `(${categoryCount} of ${totalCategoryCount} category(s) will be enabled)`;
        setSelectedAction("enable");
        break;
      case "pause":
        actionPhrase = `(${categoryCount} of ${totalCategoryCount} category(s) will be paused)`;
...
export default CategoryActionHeader;
