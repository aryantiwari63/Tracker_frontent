...
const CampaignActionHeader = ({ campaignSelection, changeTagsData, account, setShowBulkPopup }) => {
  ...
  let campaignCount = selectedCheckBox?.campaign?.length;
  ...
  const handleApplyButton = () => {
    ...
    const data = selectedCheckBox?.campaign?.map((data) => {
      if (selectedAction === "pause" || selectedAction === "enable") {
        ...
        if (budgetType === "total") {
          message = `Set budget to ${currency}${budget} from ${data?.campaign_budget}`;
          set_value = parseFloat(budget);
        } else if (budgetType === "amount") {
          if (budgetVariation === "increase") {
            message = `Increase budget by ${currency}${budget} from ${data?.campaign_budget}`;

            set_value = numericBudget + parseFloat(budget);
          } else {
            message = `Decrease budget by ${currency}${budget} from ${data?.campaign_budget}`;
            set_value = numericBudget - parseFloat(budget);
          }
        } else if (budgetType === "percentage") {
          if (budgetVariation === "increase") {
            message = `Increase budget by ${budget}% from ${data?.campaign_budget}`;
            set_value =
              numericBudget + (numericBudget * parseFloat(budget)) / 100;
          } else {
            message = `Decrease budget by ${budget}% from ${data?.campaign_budget}`;
            set_value =
              numericBudget - (numericBudget * parseFloat(budget)) / 100;
          }
        }
    ...
    if (
      ...
      setError("Budget must be greater than ₹100");
    } else {
      handleAction(data);
      setError(false);
    }
  ...
  const handlePinClick = async () => {
    ...
    await _POST(ZEPTO_CAMPAIGN_PIN, data);
    campaignSelection(false);
    dispatch({
      type: ActionType.RECALLCAMPAIGNPAPI,
      payload: true,
