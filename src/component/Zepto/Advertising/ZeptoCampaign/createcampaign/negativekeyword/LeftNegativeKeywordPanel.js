...
const LeftNegativeKeywordPanel = ({
  ...
  const { addedKeywords, setAddedKeywords, matchType, setMatchType } =
    useContext(NegativeKeywordsContext);
  ...
  const addSelectedKeywords = () => {
    ...
    const removedKeyData =
      ...
      campaignData?.keywords.map((data) => data.keyword.toLowerCase());
    ...
    keywordData &&
      ...
      keywordData.map((data) => {
        if (
          ...
          campaignData?.negativeKeywords.map((key) => {
            if (
              key.keyword.toLowerCase() == data.keyword.toLowerCase() &&
              key.matchType == data.matchType
            )
    ...
    if (duplicateKeyword && duplicateKeyword?.length) {
      let newKeywordData =
        ...
        campaignData?.keywords
          .map((data) => {
            if (!duplicateKeyword.includes(data.keyword.toLowerCase()))
              return data;
          })
          .filter((data) => data != undefined);
        ...
        for (let j = 0; j < existingKeyword.length; j++) {
          if (
            key.keyword.toLowerCase() ===
              existingKeyword[j].keyword.toLowerCase() &&
  ...
  const handleDialogCancel = () => {
    if (duplicateKeyword && duplicateKeyword?.length) {
      ...
      searchTerm &&
        searchTerm.length &&
        searchTerm?.split(",").map((keyword) => {
          if (!duplicateKeyword.includes(keyword.toLowerCase()))
            keywordData.push({ keyword, matchType });
        });
      setAddedKeywords([...addedKeywords, ...keywordData]);
    }
  ...
  // React.useEffect(() => {
  //   console.log("campaignData searchTerm", searchTerm, searchTerm?.length);
  // }, [searchTerm]);
  return (
    <>
      <div className="col_6 px-3 ">
        ...
        <div className="py-5">
          ...
          />
          {/* {error.negativeKeywords && (
            <p className="errorText">{error.negativeKeywords}</p>
          )} */}
        </div>
