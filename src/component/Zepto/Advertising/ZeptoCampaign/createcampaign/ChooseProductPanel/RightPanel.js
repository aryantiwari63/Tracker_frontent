...
const RightPanel = ({
  ...
  const delProduct = (v) => {
    setSelectedProduct(
      selectedProduct.filter((item) => item.product_id !== v.product_id)
    );
  };
  ...
  //   console.log("campaignData category>>>>>>>>", campaignData?.category);
  ...
  React.useEffect(() => {
    // eslint-disable-next-line no-console
    // console.log("campaignData values>>>>>>>>", values, campaignData?.category);
