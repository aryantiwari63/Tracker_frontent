...
const CreativeReport = () => {
  ...
  const campaignReportDownloadApi = async () => {
    ...
    setDownloadHeader(headersDownload);
      const res = await _POST(CREATIVE_REPORT_DOWNLOAD_API_URL, downloadFilter,{responseType: "arraybuffer",
      headers: {"Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"}});
    const blob = new Blob(["\ufeff", res.data], { type: 'text/csv;charset=utf-8' });
      const pom = document.createElement('a');
      pom.href = URL.createObjectURL(blob);
      pom.setAttribute('download', `Creative report_${downloadFilter.account}_${downloadFilter.start_date}-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`);
      pom.click();
