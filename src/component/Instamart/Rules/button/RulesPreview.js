...
import "./style.css";
import { campaignHeaders, adgroupHeaders, keywordHeaders, fsnHeaders, productHeaders } from "../../../../utils/rulesConstants"
...
const RulesPreview = ({ ruleId, setShowPopup, showPopup, getRulesApi }) => {
    ...
    useEffect(() => {
        ...
        if (ruleId.entity == 'Campaign') {
            setHeaders(campaignHeaders.filter((item) => item.value != "segment"));
        }
        if (ruleId.entity == 'AdGroup') {
            setHeaders(adgroupHeaders);
        }
        if (ruleId.entity == 'Keyword') {
            setHeaders(keywordHeaders.filter((item) => item.value != "segment"));
        }

        if (ruleId.entity == 'FSN') {
            setHeaders(fsnHeaders);
        }
        if (ruleId.entity == 'Product') {
            setHeaders(productHeaders.filter((item) => item.value != "segment"));
        }
    ...
    function getCustomButton() {
        ...
        if (hasPermission) {
            customButton.push({
                ...
                label: "Save Changes",
                style: `${disable ? " text-white font-semibold py-2 px-4 opacity-50 cursor-not-allowed" : "text-white"} border rounded bg-[#851853]`,
                disabled: disable 
            });
        } 
        return customButton;   
                    ...
                    <tr>
                        <th scope="col" className="p-[1.1rem]" style={{ minWidth: '40px' }}>
                            <div className="flex items-center">
                                <input checked={resultData?.length == selected?.length && resultData?.length > 0} onChange={(e) => { handleAllCheckBox(e) }} id="checkbox-all" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-pink-800 dark:focus:ring-pink-900 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 accent-pink-800" disabled={!hasPermission} />
                        ...
                        resultData && resultData?.length > 0 ? 
                         resultData.map((item, index) => (<tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td className="w-4 p-4">
                                <div className="flex items-center">
                                    <input onChange={(e) => { handleCheckboxChange(e.target.checked, item.id) }} checked={selected?.includes(item?.id)} id="checkbox-table-1" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-pink-800 dark:focus:ring-pink-900 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 accent-pink-800" disabled={!hasPermission} />
                            ...
                            {headers && headers.map((item1, index1) => {
                                if (item1.value == 'platform_id') {
                                    return (
                                        <td key={index1} className="px-6 py-4">
                                            {item.account}
                                        </td>
                                    )
                                }
                                if (item1.value == 'platform') {
                                    return (
                                        <td key={index1} className="px-6 py-4">
                                            {item.platform ? item.platform : "Instamart"}
                                        </td>
                                    )
                                }
                                else {
                                    ...
                                    }} scope="row" className=" px-6 py-4 font-medium text-gray-900 dark:text-white">
                                        {item[item1.value]}
                                    </td>)
                                }
                            })}
