...
import { instamartRuleHeaders, PERMISSIONS } from "../../../utils/constants";
import { _GET, _POST } from "../../../services/axios.method";
import { GET_RULES, INSTAMART_BRANDS } from "../../../utils/constants";
...
const Rules = () => {
  ...
  // eslint-disable-next-line no-unused-vars
  const [showHeader, setShowHeader] = React.useState([...instamartRuleHeaders]);
  ...
  React.useEffect(() => {
    ...
    dispatch(instamartWalletBal(accounts));
  }, [platformId]);
  ...
  return (
    <>
      ...
      <section className="mt-4 flex-nowrap pt-4 pl-2  bg-white">
        <div className="flex">
          ...
          <div className="relative text-gray-600 ml-2">
            <input type="search" value={search} onChange={(e) => { setSearch(e.target.value) }} name="serch" placeholder="Search by Name" className=" w-96 border border-[#ccc] bg-white h-8 px-5 pr-10  text-sm focus:outline-none focus:border-[#851853] focus:ring-[#851853] rounded-md sm:text-sm focus:ring-1" />
      ...
      <section>
        ...
        <WhenPermitted platform="instamart" permission={PERMISSIONS.CREATE_RULE}>
          <button
            className="card__btn row mb-4 instamart_btn"
