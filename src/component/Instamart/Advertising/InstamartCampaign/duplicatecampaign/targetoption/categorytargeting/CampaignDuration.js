...
const CampaignDuration = ({
  ...
  const slotoptions = [
    ...
    { label: "Late Night", value: "MEAL_SLOT_LATE_DINNER" },
  ];
  ...
  // const [initailSet, setInitailSet] = React.useState(true);
  ...
  React.useEffect(() => {
    ...
    //   setInitailSet(false);
    // }
    if (campaignData.start_duration < new Date().toISOString().slice(0, -14)) {
      setCampaignData({
        ...campaignData,
        start_duration: new Date().toISOString().slice(0, -14),
      });
