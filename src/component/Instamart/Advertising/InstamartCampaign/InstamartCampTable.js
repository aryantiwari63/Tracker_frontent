...
import InstamartSearchTerm from "../../../common-components/InstamartSearchTable";
import {
  getKeywordList,
  getProductList,
  getCampaignList,
} from "../../../../redux/action-creator/instamart/instamartCampaignAction";
import { convertDate } from "../../../../utils/helpers";
import { CSVDownload } from "react-csv";
import { connect } from "react-redux";
import ActionType from "../../../../redux/types";

class InstamartCampTable extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      headers: [],
  ...
  async initProcess() {
    ...
    let payload = {
      ...
      account: this.props.account || "Durex",
    };
    ...
    if (name === "campaign") {
      ...
      expandState = await getProductList(payload);
    }
    if (this.props.download == 1 && expandState) {
      ...
      if (expandState && expandState.data && expandState.data.length > 0) {
        this.setState(
          {
...
const mapStateToProps = (state) => {
  return {
    ...
    // instamartFunnelCount: state.CampaignReducer.instamartFunnelCount,
  };
};
