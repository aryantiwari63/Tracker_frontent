...
import {
  ...
  INSTAMART_CAMPAIGN_REPORT_DOWNLAOD,
  LIMIT,
  instamartCampaignReport,
...
const CampaignReport = () => {
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(INSTAMART_CAMPAIGN_REPORT, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      pom.setAttribute(
        "download",
        `campaign_report_${downloadFilter.start_date}-${
          downloadFilter.end_date
        }_${downloadFilter.date}_${Date.now()}.csv`
      );
      pom.click();
  ...
  React.useEffect(() => {
    if (selectedPlatformVal !== "") {
      dispatch(instamartWalletBal([selectedPlatformVal]));
    }
  }, [selectedPlatformVal]);
  ...
  const modifiedBodyContent = campaignData?.map((item) => {
  ...
  //     dispatch(instamartWalletBal("Dettol"));
  //   }
  // }, [selectedAccountVal]);
  return (
    <>
      ...
      <section className="py-7 ">
        ...
        <div className="bg-white px-3">
          <FlipkartTable
            headers={showHeader}
            bodyContent={modifiedBodyContent}
            sortBy={sortBy}
            loading={loading}
            sortData={sortData}
            paginate={paginate}
