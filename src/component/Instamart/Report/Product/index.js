...
import {
  ...
  INSTAMART_PRODUCT_REPORT_DOWNLOAD,
  LIMIT,
  instamartProduct,
...
const InstamartProductReport = () => {
  ...
  const [showHeader, setShowHeader] = React.useState([...instamartProduct]);
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(INSTAMART_PRODUCT_REPORT, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      pom.setAttribute(
        "download",
        `product_report_${downloadFilter.start_date}-${
...
export default InstamartProductReport;
