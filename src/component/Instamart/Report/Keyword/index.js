...
import {
  INSTAMART_KEYWORD_REPORT,
  LIMIT,
  instamartKeyword,
  INSTAMART_KEYWORD_REPORT_DOWNLOAD,
...
const KeywordReport = () => {
  ...
  const [showHeader, setShowHeader] = React.useState([...instamartKeyword]);
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(INSTAMART_KEYWORD_REPORT, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      pom.setAttribute(
        "download",
        `keyword_report_${downloadFilter.start_date}-${
