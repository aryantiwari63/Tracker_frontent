...
import {
  LIMIT,
  AMAZON_KEYWORD_REPORT_DOWNLOAD_API_URL,
  keywordHeaders,
  downloadsearchTermsHeaders,
  amazonKeywordHeaders,
  MAZON_KEYWORD_REPORT_DOWNLOAD_API,
...
const KeyReport = () => {
  ...
  const [showHeader, setShowHeader] = React.useState([...amazonKeywordHeaders]);
  ...
  const cancelFilter = () => {
    setShowHeader([...amazonKeywordHeaders]);
    setShowFilter(false);
  };
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST("amazon/keywordReport", post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      pom.setAttribute(
        "download",
        `Keyword report_${downloadFilter.account}_${
          downloadFilter.start_date
        }-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`
      );
      pom.click();
  ...
  }, [selectedAccountVal, selectedPlatformVal, selectedTypeVal, sortBy]);

  React.useEffect(() => {
    setResetData(false);

    if (callApi) {
      campaignReportApi(false);
    }
  }, [dataLIMIT]);
