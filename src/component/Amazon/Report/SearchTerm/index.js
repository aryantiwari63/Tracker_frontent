...
const AmazonSearchReport = () => {
  ...
  const cancelFilter = () => {
    setShowHeader([...amazonSearchTermsHeaders]);
    setShowFilter(false);
  };
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      setLoading(true);
      const headersDownload = [{ label: "Date", key: "created_on" }];
      ...
      const res = await _POST("amazon/getSearchReport", post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      const res = await _POST(AMAZON_SEARCH_REPORT_DOWNLOAD, downloadFilter, {
      ...
      pom.setAttribute(
        "download",
        `Search term report_${downloadFilter.account}_${
...
export default AmazonSearchReport;
