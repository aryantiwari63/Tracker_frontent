...
import {
  ...
  amazonreportcampaignheader,
  CAMPAIGN_REPORT_DOWNLOAD_URL,
...
const CampaignList = () => {
  ...
  const cancelFilter = () => {
    setShowHeader([...amazonreportcampaignheader]);
    setShowFilter(false);
  };
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST("amazon/campaign_list", post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      setDownloadHeader(headersDownload);
      const res = await _POST(CAMPAIGN_REPORT_DOWNLOAD_URL, downloadFilter,{responseType: "arraybuffer",
      headers: {"Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"}});
    const blob = new Blob(["\ufeff", res.data], { type: 'text/csv;charset=utf-8' });
      const pom = document.createElement('a');
      pom.href = URL.createObjectURL(blob);
      pom.setAttribute('download', `Campaign report_${downloadFilter.account}_${downloadFilter.start_date}-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`);
      pom.click();
