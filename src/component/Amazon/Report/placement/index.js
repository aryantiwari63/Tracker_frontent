...
const AmazonPlacementReport = () => {
  ...
  const cancelFilter = () => {
    setShowHeader([...amazonplacementheader]);
    setShowFilter(false);
  };
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(AMAZON_PLACEMENT_REPORT, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      pom.setAttribute(
        "download",
        `Placement report_${downloadFilter.account}_${
...
export default AmazonPlacementReport;
