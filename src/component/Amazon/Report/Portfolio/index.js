...
const PortfolioList = () => {
  ...
  const cancelFilter = () => {
    setShowHeader([...amazonPortfolioheader]);
    setShowFilter(false);
  };
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST("/amazon/portfolioReport", post, {
  ...
  const ReportDownloadApi = async () => {
    try {
      ...
      setDownloadHeader(headersDownload);
      setDownloadHeader(headersDownload);
      const res = await _POST("/amazon/portfolioReport", downloadFilter, {
      ...
      pom.setAttribute(
        "download",
        `Portfolio report_${downloadFilter.account}_${
