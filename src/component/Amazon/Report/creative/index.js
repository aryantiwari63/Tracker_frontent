...
import {
  AMAZON_CREATIVE_REPORT,
  LIMIT,
  amazoncreativeheader,
  AMAZON_CREATIVE_REPORT_DOWNLOAD_API_URL,
...
const AmazonCreativeReport = () => {
  ...
  const [showHeader, setShowHeader] = React.useState([...amazoncreativeheader]);
  ...
  const cancelFilter = () => {
    setShowHeader([...amazoncreativeheader]);
    setShowFilter(false);
  };
  ...
  const campaignReportApi = async (date = false) => {
    ...
    try {
      ...
      const res = await _POST(AMAZON_CREATIVE_REPORT, post, {
  ...
  const campaignReportDownloadApi = async () => {
    try {
      ...
      pom.setAttribute(
        "download",
        `Creative report_${downloadFilter.account}_${
...
export default AmazonCreativeReport;
