...
const AsinList = () => {
	...
	const [showHeader, setShowHeader] = React.useState([...amazonAsinheader]);
	...
	const cancelFilter = () => {
		setShowHeader([...amazonAsinheader]);
		setShowFilter(false);
	};
	...
	const campaignReportApi = async (date = false) => {
		...
		try {
			...
			const res = await _POST('/amazon/asinReport', post, {
	...
	const ReportDownloadApi = async () => {
		try {
			const ourRequest = await cancelRequest();
			// const notIncluded = ["portfolio_name", "campaign_name", "ad_group_name"];
			...
			// console.log("showFiltereed:::::::::::::::::", filteredHeaders);
			headersDownload.push(...filteredHeaders);
			...
			const res = await _POST('/amazon/asinReport', downloadFilter,{responseType: "arraybuffer",
			headers: {"Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"}});
			  const blob = new Blob(["\ufeff", res.data], { type: 'text/csv;charset=utf-8' });;
			const pom = document.createElement('a');
			pom.href = URL.createObjectURL(blob);
			pom.setAttribute('download', `ASIN report_${downloadFilter.account}_${downloadFilter.start_date}-${downloadFilter.end_date}_${downloadFilter.date}_${Date.now()}.csv`);
			pom.click();
