...
import { metricAnalyzerAdgroup } from "../../../../services/dashboard";
import LoaderSpinner from "../../../common-components/loader-spinner";

const TopPerformingAdgroupTable = ({
  headers,
  platform,
  selectedMetric,
  campType,
    ...
    try {
      if (callApi || reset) {
        if (platform.length > 0) {
          ...
          filters.breakdown = topBreakdown
            .map((item) => {
              if (item.checked) {
                return item.value;
              }
          ...
          if (reset) {
            await setAdgroupdata([...dashData.data.data.data]);
          } else {
            await setAdgroupdata([...adgroupdata, ...dashData.data.data.data]);
          }
          if (dashData.data.data.data.length === 50) {
            setCallApi(true);
          } else {
            setCallApi(false);
          }
            ...
            {adgroupdata?.map((item, i) => (
              <tr key={i}>
                {headers?.map((headerItem, i) => (
                  <td key={i}
                    className={
                      headerItem.id < 3 ? "metricanalyzerCampaignName" : ""
                    }
...
export default TopPerformingAdgroupTable;
