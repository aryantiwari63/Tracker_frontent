...
//import { campaigndata } from "../../../../utils/amazonConstants";

import LoaderSpinner from "../../../common-components/loader-spinner";

const TopPerformingCampaignTable = ({
  headers,
  loading,
  platform,
  dataLIMIT,
  ...
  return (
    ...
    >
      <table className="text-left  campaignsTable ">
                    ...
                    style={{
                      width:
                        item.size === "l"
                          ...
                          : item.size === "xl"
                          ? 110
                          : item.value === "sales" || item.value === "spend" || item.value ==="impressions"
                          ? 90
                ...
                {headers.map(
                  (header, headerIndex) =>
                    header.showCol && (
                      <td
                        className={`${
                          headerIndex==0 ? "metricanalyzerCampaignName" : ""
                        }`}
                        key={headerIndex}
...
export default TopPerformingCampaignTable;
