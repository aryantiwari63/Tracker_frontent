        ...
        >
          <div className="flex justify-center">
            {error && (
              <div className=" text-center flex">
                {cardName === "overview" && tempView.length !== 9 ? (
                  <div className="flex">
                    <img
                      className="w-5 mr-1"
            ...
            {Object.keys(allData).map((item, i) => {
              if (tempView.indexOf(item) > -1) {
                return (
                  ...
                  >
                    <Box platform="amazon" title={allData[item].name} />
                  </div>
                );
              }
            })}
          ...
          <div className="row bg-bgclr p-4">
            {Object.keys(allData).map((val, key) => {
              ...
              let active =
                tempView.indexOf(val) > -1 ? "border-orange-500" : "";
              return (
                <div
                  key={key}
                  className="col_2 p-3"
                ...
                >
                  <div
                    className={`bg-bgclr py-3 px-5 text-center border ${active} rounded-md 
                    cursor-pointer`}
                  >
...
export default AmazonSummaryViewPopup;
