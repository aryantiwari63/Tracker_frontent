...
const RightNegativeProductTargeting = () => {
  const {handleDeleteItem,handleRemoveAllItems,selectedItems,} = useContext(EventHandlerContext)
  return (
    <>
      <div className="w-full max-w-screen-md px-4 mt-8 font-semibold">
        ...
        <div className='border-t border-b border-gray-200 py-4 font-normal'>Brands & products</div>
        <ul>
          {selectedItems?.map((item, index) => (
            <li key={index} className="row items-center justify-between py-4">
              <span>{item.label}</span>
              <button
                className={`ml-2 text-gray-500 hover:text-red-800 focus:outline-none`}
...
export default RightNegativeProductTargeting;
