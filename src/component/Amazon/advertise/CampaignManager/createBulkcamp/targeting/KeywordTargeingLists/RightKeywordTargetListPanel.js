...
const RightKeywordTargetListPanel = ({
  ...
  React.useEffect(() => {
    ...
    if (data && data[formIndex]) {
      data[formIndex]["keywords"] = addedProducts;
    }
  ...
  React.useEffect(() => {
    if (
      ...
      campaignData[0] &&
      formIndex > 0 &&
      campaignData[0]?.keywords
    ) {
      // eslint-disable-next-line no-unsafe-optional-chaining
      setAddedProducts([...campaignData[0]?.keywords]);
    }
  }, [formIndex]);
  const handleBID = (data, keyword, match) => {
    let tempData = campaignData[formIndex]?.keywords;
    ...
    if (newdata && newdata[formIndex]) {
      newdata[formIndex]["keywords"] = result;
    }
              ...
              <tr className="border-b " key={i}>
                ...
                <td className="py-2 ">
                  <input
                    ...
                    type="number"
                    value={
                      campaignData[formIndex]?.keywords?.filter(
                        (data) =>
                          data.keyword == item?.keyword &&
                          data.match == item?.match
                      )[0]?.bid
