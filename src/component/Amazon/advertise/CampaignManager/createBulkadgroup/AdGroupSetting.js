...
const AdGroupSetting = ({
  ...
  const Minimize = (formIndex) => {
    setShowAdgroup([...showAdgroup, formIndex]);
  };
  const Maximize = (formIndex) => {
    let data = [...showAdgroup];
    let updatedData = data.filter((data) => data != formIndex);

    setShowAdgroup([...updatedData]);
  };
  // React.useEffect(() => {
  //   console.log("campaignData showAdgroup", showAdgroup);
  // }, [showAdgroup]);
