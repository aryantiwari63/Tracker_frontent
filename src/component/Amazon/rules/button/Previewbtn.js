...
const PreviewBtn = ({ rowData, platform = "" }) => {
  ...
  console.log('1234', hasPermission);
  ...
  const updateEmails = async () => {
    try {
      ...
      let emailChecker = validEmail.every((v) => v === true || v === undefined);
  ...
  const buttonClass =
    platform === "ams"
      ? "hover:!bg-[#EF880F] hover:border-[#EF880F]"
      : platform === "blinkit"
      ? ""
      : "";
    ...
    <>
      <div className="flex justify-center items-center gap-2">
        <div className="flex items-center justify-center">
          <button
            className={`preview__btn ${buttonClass}`}
          ...
          >
            <div className="flex flex-col items-center justify-center">
              ...
              <div className="col pl-1">Preview</div>
            </div>
          </button>
        </div>
        <div className="relative" ref={dropdownRef}>
        <WhenPermitted permission={PERMISSIONS.CREATE_RULE} platform="amazon">
          <button className={`preview__delbtn hover:text-white ${buttonClass}`} onClick={() => setOpen(!open)}>
