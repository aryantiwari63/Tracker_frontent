...
import { getByBoxSellerSettings, updateByBoxSellerSettings, updateByBoxSellerAutoAssigned } from "../../../services/ebuxMaster.service";
...
export default function ByBoxSellerSettings() {
  ...
  const [selectedAutoAssignedSeller, setSelectedAutoAssignedSeller] = useState("");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await getByBoxSellerSettings(page, pageSize, filterType, searchTerm);
      ...
      setSelectedAutoAssignedSeller(res?.is_seller_type_autoset ?? false);

      // ✅ backend total records
      setTotalRecords(pagination?.totalRecord || 0);
  ...
  //   return tableData.filter(r => (r.seller_name || "").toLowerCase().includes(q));
  ...
  const filteredData = useMemo(() => {
    ...
    // seller type filter
    if (filterType !== "") {
      list = list.filter(r => r.seller_type === filterType);
    }
  ...
  const sortedData = useMemo(() => {
    // sort the filtered data (so search sort combine)
  ...
  const sellerCards = [
    ...
    { id: 4, title: "Un-Assigned Seller", subtitle: "Un-Assigned Party", icon: "/assets/images/master/monitor-up.svg", value: "countNull", change: "5% from last month", changeType: "down" },
  ];
  ...
  const handleDownload = async () => {
    try {
      ...
      const res = await getByBoxSellerSettings(
        1,                 // page
        totalRecords,      // limit = ALL records
        filterType,
        searchTerm
      );
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `buy_box_seller_settings_${Date.now()}.xlsx`;
      link.click();
  ...
  const handleChange = async (value) => {
    setSelectedAutoAssignedSeller(value)
    const payload = {
      value: value == true ? 1 : 0
    }
    const result = await updateByBoxSellerAutoAssigned(payload);
    ...
    <div className="">
      ...
      <div className="bg-gray-50 mt-2 px-4 flex flex-col">
        <main className="flex-1 px-4 bg-white rounded-lg shadow-md py-2">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition ${downloading ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-100"} `}
          ...
          <div className="flex gap-4 items-center my-4 px-2 relative">
            ...
            <div className="absolute right-[20px] flex gap-2">
              <p>Auto Assign New Sellers to 3P : </p>
              <button
                // onClick={() => setSelectedAutoAssignedSeller((v) => !v)}
                onClick={() => handleChange(!selectedAutoAssignedSeller)}
                aria-pressed={selectedAutoAssignedSeller}
                className={`
          relative inline-flex h-[22px] w-[42px] items-center rounded-full transition
          ${selectedAutoAssignedSeller ? "bg-blue-600" : "bg-neutral-300"}
        ...
        `}
            ...
            ${selectedAutoAssignedSeller ? "translate-x-[22px]" : "translate-x-[4px]"}
        ...
        <div className="flex items-center justify-between mt-4 mb-[120px]">
