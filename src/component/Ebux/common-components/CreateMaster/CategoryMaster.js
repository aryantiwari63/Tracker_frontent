...
import { getPlatformList, createCategoryMaster } from "../../services/ebuxMaster.service";
...
export default function CategoryMaster() {
  ...
  const toggleSelectAll = () => {
    ...
    const allSelected = pf_ids.length === allIds.length && allIds.length > 0;
    setValue("pf_ids", allSelected ? [] : allIds, { shouldValidate: true, shouldDirty: true });
  };

  // toggles for status (0/1)
  ...
  const onSubmit = async (formData) => {
    ...
    setSuccessMsg("");
    try {
      // adapt payload shape if backend expects different keys
      const payload = {
        category_name: formData.category_name,
        // alias: formData.alias,
        pf_ids: formData.pf_ids,
        status: Number(formData.status ?? 0),
      };

      const result = await createCategoryMaster(payload);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="p-4 gap-6">
            ...
            <div className="mt-4 flex flex-wrap gap-4">
              {/* Category Name */}
              <div className="w-[40%]">
                <label className="font-inter font-medium text-[14px]">Category Name</label>
                <input
                  type="text"
                  ...
                  disabled={loading}
                />
                {errors.category_name && <p className="text-sm text-red-600 mt-1">{errors.category_name.message}</p>}
              </div>
                ...
                {isDropdownOpen && (
                  <div className="absolute z-10 w-full bg-white border rounded shadow mt-1 max-h-60 overflow-auto" role="listbox" aria-label="Platforms">
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={pf_ids.length === platforms.length && platforms.length > 0}
                        readOnly
                      />
                      <span className="ml-2">{pf_ids.length === platforms.length && platforms.length > 0 ? "Unselect All" : "Select All"}</span>
                    </div>
