...
import { fetchComprehensiveBreakdownData } from '../../../services/comprehensiveBreakdownCS.services';
...
const ComprehensiveBreakdownComponent = ({ listeners, attributes,platformSubCat }) => {
    ...
    const [csTabPosition, setCSTabPosition] = useState([]);
    const [selectedTabName, setSelectedTabName] = useState('');

    const savePlatformPositions = (data) => {
        const _csTabPosition = data?.map((item, index) => ({
            value: item.value,
            position: index + 1
        }));
        // localStorage.setItem("CSTabPosition", JSON.stringify(_csTabPosition));
        let tabsPayload = {
            "type": 'cs_plateform',
            "key": 'ComprehensivePosition',
            "ComprehensivePosition": _csTabPosition,
          }
    ...
    const loadAndRearrangePlatformPositions = (data) => {
        ...
        // const storedPosition = JSON.parse(localStorage.getItem("CSTabPosition"));
    ...
    useEffect(() => {
        if (platform) {
        let _csTabPosition = tab_headers.map((item, index) => ({
            value: item,
            position: index + 1
        }));
        ...
        if (!storedPosition || storedPosition.length === 0) {
            savePlatformPositions(_csTabPosition ?? []);
        } else {
            _csTabPosition = loadAndRearrangePlatformPositions(_csTabPosition ?? []);
        }
        setCSTabPosition(_csTabPosition);
        setSelectedTabName(_csTabPosition?.[0]?.value)
    }
    }, [platform]);
    const handleDragEnd = (event) => {
        ...
        if (over && active?.id !== over?.id) {
            setCSTabPosition((prevPosition) => {
                ...
                const _csTabPosition = newdata.map((item, index) => ({
                    value: item.value,
                    position: index + 1
                }));
                // localStorage.setItem("CSTabPosition", JSON.stringify(_csTabPosition));
                let tabsPayload = {
                  ...
                  saveOSAPlateform(tabsPayload);
                const activeIndex = _csTabPosition.findIndex((item) => item.value === selectedTabName);
    ...
    useEffect(() => {
        const loadPlatformData = async () => {
          try {
            ...
            // }
            if (platformSubCat?.cs_plateform?.ComprehensivePosition) {
                setPlatform(platformSubCat.cs_plateform);
              }
          } catch (error) {
    ...
    const handleTabSelect = (index) => {
        ...
        setSelectedTabName(csTabPosition?.[index]?.value);
    };
    return (
        <>
                                    ...
                                    <div className='tabListWrap'>
                                        <div className='tabList'>
                                            <DndContext sensors={sensors} onDragEnd={handleDragEnd} >
                                                <SortableContext items={csTabPosition.map((item) => item.value)} >
                                                    {csTabPosition?.length && csTabPosition?.map((item) => (
                                                        <Tab key={item.value}>
                                                            <SortableTab
                                                                key={item.value}
                                                                id={item.value}
