...
import { fetchComprehensiveBreakdownData } from '../../../services/comprehensiveBreakdownOR.services';
...
const ComprehensiveBreakdownComponent = ({ listeners, attributes,platformSubCat }) => {
    ...
    const [orTabPosition, setORTabPosition] = useState([]);
    const [selectedTabName, setSelectedTabName] = useState('');

    const savePlatformPositions = (data) => {
        const _orTabPosition = data?.map((item, index) => ({
            value: item.value,
            position: index + 1
        }));
        // localStorage.setItem("ORTabPosition", JSON.stringify(_orTabPosition));
        let tabsPayload = {
            "type": 'org_plateform',
            "key": 'ComprehensivePosition',
            "ComprehensivePosition": _orTabPosition,
          }
    ...
    const loadAndRearrangePlatformPositions = (data) => {
        ...
        // const storedPosition = JSON.parse(localStorage.getItem("ORTabPosition"));
...
console.log('org-fnal arr',finalArray)
    ...
    useEffect(() => {
        if (platform) {
        let _orTabPosition = tab_headers.map((item, index) => ({
            value: item,
            position: index + 1
        }));
        ...
        if (!storedPosition || storedPosition.length === 0) {
            console.log('223',storedPosition)
            savePlatformPositions(_orTabPosition ?? []);
        } else {
            console.log('2234',storedPosition)
            _orTabPosition = loadAndRearrangePlatformPositions(_orTabPosition ?? []);
        }
        setORTabPosition(_orTabPosition);
        setSelectedTabName(_orTabPosition?.[0]?.value)
    }
    }, [platform]);
    const handleDragEnd = (event) => {
        ...
        if (over && active?.id !== over?.id) {
            setORTabPosition((prevPosition) => {
                ...
                const _orTabPosition = newdata.map((item, index) => ({
                    value: item.value,
                    position: index + 1
                }));
                // localStorage.setItem("ORTabPosition", JSON.stringify(_orTabPosition));
                ...
                const activeIndex = _orTabPosition.findIndex((item) => item.value === selectedTabName);
    ...
    useEffect(() => {
        const loadPlatformData = async () => {
          try {
            ...
            // }
            if (platformSubCat?.org_plateform?.ComprehensivePosition) {
                setPlatform(platformSubCat.org_plateform);
              }
          } catch (error) {
    ...
    const handleTabSelect = (index) => {
        ...
        setSelectedTabName(orTabPosition?.[index]?.value);
    };
    return (
        <>
                                    ...
                                    <div className='tabListWrap'>
                                        <div className='tabList'>
                                            <DndContext sensors={sensors} onDragEnd={handleDragEnd} >
                                                <SortableContext items={orTabPosition.map((item) => item.value)} >
                                                    {orTabPosition?.length && orTabPosition?.map((item) => (
                                                        <Tab key={item.value}>
                                                            <SortableTab
                                                                key={item.value}
                                                                id={item.value}
