...
const SelectedItemPopupComponent = (
  ...
  const filteredData = useMemo(() => {
    ...
    if (keywordQuery) {
      return values.filter(item => item?.lable?.toLowerCase().includes(keywordQuery?.toLowerCase()));
          ...
          <div className="flex flex-wrap gap-2 px-4">
            ...
            {filteredData.map((item, index) => (
              <button
                ...
                key={index}
                className={`categoryTagsBox ${localSelectedValues.includes(item) ? 'selected' : 'disabled'} `}
              >
                <label >{item?.lable}</label>
                {localSelectedValues.includes(item) && (
                  <span>
