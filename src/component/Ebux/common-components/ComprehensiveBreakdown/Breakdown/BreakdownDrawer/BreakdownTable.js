import { useEffect, useMemo, useState } from "react";
import Loader from "../../../Loader";
import SortingButton from "../../DynamicSortButton";
import { useEbuxContext } from "../../../../Context/EbuxProvider";
import Excel from "exceljs";

// import { LuIndianRupee } from "react-icons/lu";
const BreakDownTable = ({ drawerInfo, reportTableData, breakdownPerformance = {} }) => {
  const {
    kpi,
    // percentageIcon,
    filters,
    activeClientProject
  } = useEbuxContext();


  // const headerkey = { "output": "Average OSA" };
  const [headerkey, setHeaderkey] = useState({});
  useEffect(() => {
    if (breakdownPerformance?.ColumnList?.length) {
      const _data = {}
      breakdownPerformance?.ColumnList?.forEach((col) => {
        if (col?.value)
          _data[col?.value] = {...col};
          // _data[col?.value] = col?.title ?? col?.value;
      })
      setHeaderkey(_data);
    }

  }, [breakdownPerformance]);
  const [rowData, setRowData] = useState([]);
  const [footerData, setFooterData] = useState({});
  useEffect(() => {
    
    if (reportTableData?.length) {
      const row = reportTableData.slice(0, -1);
      setRowData(row);

      const footerd = reportTableData.slice(-1);
      setFooterData(footerd[0]);
  }

  }, [reportTableData]);

  // const pfkey = ["avg", "Amazon", "Zepto"];
  let pf = filters?.platform?.length ? filters?.platform?.map((i) => i?.label ?? "") : [];
  const pfkey = ["avg", ...[...new Set(pf ?? [])]];

  const [loading, setLoading] = useState(false);
  const [sortConfig, setSortConfig] = useState({ key: null, direction: 'ascending' });
  const sortedData = useMemo(() => {
    if (sortConfig.key) {
      return [...rowData].sort((a, b) => {
        const _sortkey = sortConfig?.key.split(".");
        a = sortConfig.key == 'date' ? (a?.[sortConfig.key]) : (a?.[_sortkey[0]]?.[_sortkey[1]]);
        b = sortConfig.key == 'date' ? (b?.[sortConfig.key]) : (b?.[_sortkey[0]]?.[_sortkey[1]]);

        if (a === undefined || a === null) return 1;
        if (b === undefined || b === null) return -1;

        if (!isNaN(a) && !isNaN(b)) {
          a = parseFloat(a);
          b = parseFloat(b);
        }
        if (a < b) {
          return sortConfig.direction === 'ASC' ? -1 : 1;
        }
        if (a > b) {
          return sortConfig.direction === 'ASC' ? 1 : -1;
        }
        return 0;
      });
    }
    return rowData;
  }, [rowData, sortConfig]);
  const requestSort = key => {
    setLoading(true);
    let direction = 'ASC';
    if (sortConfig.key === key && sortConfig.direction === 'ASC') {
      direction = 'DSC';
    }
    setSortConfig({ key, direction });
    setTimeout(() => setLoading(false), 500);
  };
  const getHeaderIcon = (icon) => {
    switch (icon) {
        case "rupee":
            return "₹ ";
        default:
            return (icon)?icon+" ":"";
    }
}
  const getFormatValue=(_value,headerdata)=>{
    return `${_value!=undefined&&headerdata?.icon?getHeaderIcon(headerdata?.icon):""}${_value!=undefined?(headerdata?.value=="competition_price_rp" || headerdata?.value=="competition_price_sp"||headerdata?.value=="price_rp" || headerdata?.value=="price_sp")?Math.round(_value):_value:"-"}${_value!=undefined&&headerdata?.subValue?headerdata?.subValue:""}${_value!=undefined&&headerdata?.persentageValue?"%":""}`;
  }
  const handleDownload = () => {
    const headers = [
      { header: 'Date', key: 'date', width: 25, style: { numFmt: 'mm/dd/yyyy' } }
    ];
    pfkey.map((pf) => (
      Object.keys(headerkey).map((hk) => (
        headers.push({ header: `${pf == "avg" ? "" : pf} ${((activeClientProject?.useWeightedSOS&&headerkey?.[hk]?.title=="SOS")?"Wt. SOS":headerkey?.[hk]?.title)}`, key: `${pf}_${headerkey?.[hk]?.title}`, width: 25 })
      ))
    ))
    const title = `${drawerInfo?.column?.toUpperCase()} (${drawerInfo?.lable}) Performance Report`;

    const workbook = new Excel.Workbook();
    const worksheet = workbook.addWorksheet(title);

    

    worksheet.columns = headers;
    sortedData?.forEach((data) => {
      let _data = { date: data?.date || "-" };
      pfkey.map((pf) => (
        Object.keys(headerkey).map((hk) => {

          _data[`${pf}_${headerkey?.[hk]?.title}`] = data?.[pf]?.[hk] === undefined ? "-" : getFormatValue(data?.[pf]?.[hk],headerkey?.[hk]);
        })
      ))

      worksheet.addRow(_data);
    });


    // worksheet.addRow([]);
    // worksheet.addRow([title]);
    // worksheet.mergeCells(`A${sortedData.length + 3}:D${sortedData.length + 3}`);
    // const titleRow = worksheet.getRow((sortedData.length + 3));
    // titleRow.font = { size: 16, bold: true };
    // titleRow.alignment = { vertical: 'middle', horizontal: 'center' };



    workbook.xlsx.writeBuffer().then((buffer) => {
      const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });

      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `${kpi}_${drawerInfo?.column}_performance_${Date.now()}.xlsx`;
      link.click();
    });
  };

  // const getDataAVG = (key) => {

  //   const _key = key.split(".");
  //   let total = 0;
  //   const setToFixed = 2;
  //   const data = sortedData?.map(item => {
  //     if (item?.[_key[0]]?.[_key[1]] != undefined) {
  //       total++;
  //     }
  //     if (item?.[_key[0]]?.[_key[1]]) {
  //       return parseFloat(item?.[_key[0]]?.[_key[1]]);
  //     }
  //     return 0;
  //   });
  //   const result = data?.length ? ((data?.reduce((sum, avg) => sum + avg, 0)) / (total ?? 1)) : 0;
  //   return isNaN(result) ? '-' : `${result?.toFixed(setToFixed)} ${percentageIcon}`;

  // };

  const renderTableRows = () => {
    return sortedData?.map((data, _index) => (
      <tr key={data?.item?.value ?? data?.date ?? _index} >

        <td className="firstCol sticky left-[-1px]">
          <div className='flex flex-row gap-2 justify-between'>
            {data?.date || "-"}
          </div>
        </td>
        {
          pfkey.map((pf, i) => (
            Object.keys(headerkey).map((hk, index) =>
            (<td key={i + index}>
              {data?.[pf]?.[hk] === undefined ? "-" : getFormatValue(data?.[pf]?.[hk],headerkey?.[hk])}
            </td>)
            )
          ))
        }

      </tr>
    )
    );
  };
  const renderTabletfootRows = () => {

    return (
      <tr>
        <td  className="firstCol sticky left-[-1px]">
          <div>
            Total Days
          </div>
          <strong>{sortedData?.length}</strong>
        </td>
{/* <strong>{getDataAVG(`${pf}.${hk}`)}</strong> */}
        {
          pfkey.map((pf, i) => (
            Object.keys(headerkey).map((hk, index) => (
              <td key={i + index}>
                <div>
                  {pf == "avg" ? "" : pf} {((activeClientProject?.useWeightedSOS&&headerkey?.[hk]?.title=="SOS")?"Wt. SOS":headerkey?.[hk]?.title)}
                </div>                
                <strong>{footerData?.[pf]?.[hk] === undefined ? "-" : getFormatValue(footerData?.[pf]?.[hk],headerkey?.[hk])}</strong>
              </td>
            ))
          ))
        }
      </tr>
    );
  };
  const header = `${breakdownPerformance?.TableTitle??""} Breakdown`;
  return (
    <>
      <div className="flex flex-row gap-2 justify-between items-center p-4">
        <h1 className="text-sm font-semibold">
          {header}

          {loading && <Loader show={loading} fullScreen={false} />}

        </h1>
        <button type="button" onClick={() => handleDownload()}>
          <img
            src="/assets/images/downloadIcon.svg"
            alt="Download"
            width={18}
            height={18}
          />
        </button>
      </div>
      <div className="bg-white h-[48vh] overflow-y-scroll border-t border-[#e5e7eb]">
        <table className=" text-left w-full campaignsTable campaignsTable--strips">
          <thead>
            <tr className="bg-[#F7F7F8]">
              <th key="date" className="firstCol sticky left-[-1px]"><div className="sortingCol w-full py-2" onClick={() => requestSort('date')}>Date
                <SortingButton sortType={sortConfig.key === 'date' ? sortConfig.direction : ''} />
              </div></th>
              <>{
                pfkey.map((pf, i) => (
                  Object.keys(headerkey).map((hk, index) => (
                    <th key={hk + i + index}>
                      <div className="sortingCol w-full py-2" onClick={() => requestSort(`${pf}.${hk}`)}>{pf == "avg" ? "" : pf} {((activeClientProject?.useWeightedSOS&&headerkey?.[hk]?.title=="SOS")?"Wt. SOS":headerkey?.[hk]?.title)}
                        <SortingButton sortType={sortConfig.key === `${pf}.${hk}` ? sortConfig.direction : ''} />
                      </div>
                    </th>
                  ))
                ))
              }</>
            </tr>
          </thead>
          <tbody className="[&>*:nth-child(even)]:bg-grey_light [&>*:nth-child(odd)]:bg-white">
            {renderTableRows()}
          </tbody>

          <tfoot className="dataTableFoot">{renderTabletfootRows()}</tfoot>
        </table>
      </div>
    </>
  );
};
export default BreakDownTable;
