...
import { fetchComprehensiveBreakdownData } from '../../../services/comprehensiveBreakdownOSA.services';
...
const ComprehensiveBreakdownComponent = ({ listeners, attributes,platformSubCat }) => {
    ...
    const [osaTabPosition, setOsaTabPosition] = useState([]);
    ...
    const savePlatformPositions = (data) => {
        const _osaTabPosition = data?.map((item, index) => ({
            value: item.value,
            position: index + 1
        }));
        // localStorage.setItem("OSATabPosition", JSON.stringify(_osaTabPosition));
       
        let tabsPayload = {
            "type": 'osa_plateform',
            "key": 'ComprehensivePosition',
            "ComprehensivePosition": _osaTabPosition,
          }
    ...
    const loadAndRearrangePlatformPositions = (data) => {
        ...
        // const storedPosition = JSON.parse(localStorage.getItem("OSATabPosition"));
        const storedPosition = platform.ComprehensivePosition;
        console.log('comp-final-arr',storedPosition?.length)
    ...
    useEffect(() => {
        if (platform) {
        let _osaTabPosition = tab_headers.map((item, index) => ({
            value: item,
            position: index + 1
        }));
        ...
        console.log('hssssiii000i',storedPosition)
        if (!storedPosition) {
            savePlatformPositions(_osaTabPosition ?? []);
          } else {
            _osaTabPosition = loadAndRearrangePlatformPositions(_osaTabPosition ?? []);
          }
        
        setOsaTabPosition(_osaTabPosition);
        setSelectedTabName(_osaTabPosition?.[0]?.value)
    }
    }, [platform]);
    const handleDragEnd = (event) => {
        ...
        if (over && active?.id !== over?.id) {
            setOsaTabPosition((prevPosition) => {
                ...
                const _osaTabPosition = newdata.map((item, index) => ({
                    value: item.value,
                    position: index + 1
                }));
                // localStorage.setItem("OSATabPosition", JSON.stringify(_osaTabPosition));
                ...
                const activeIndex = _osaTabPosition.findIndex((item) => item.value === selectedTabName);
    ...
    useEffect(() => {
        const loadPlatformData = async () => {
          try {
            ...
            // }
            if (platformSubCat?.osa_plateform?.ComprehensivePosition) {
    ...
    const handleTabSelect = (index) => {
        ...
        setSelectedTabName(osaTabPosition?.[index]?.value);
    };
    return (
        <>
                                    ...
                                    <div className='tabListWrap'>
                                        <div className='tabList'>
                                            <DndContext sensors={sensors} onDragEnd={handleDragEnd} >
                                                <SortableContext items={osaTabPosition.map((item) => item.value)} >
                                                    {osaTabPosition?.length && osaTabPosition?.map((item) => (
                                                        <Tab key={item.value}>
                                                            <SortableTab
                                                                key={item.value}
                                                                id={item.value}
