...
import { fetchComprehensiveBreakdownData } from '../../../services/comprehensiveBreakdownRR.services';
...
const ComprehensiveBreakdownComponent = ({ listeners, attributes, platformSubCat }) => {
    ...
    const [rrTabPosition, setRRTabPosition] = useState([]);
    const [selectedTabName, setSelectedTabName] = useState('');

    const savePlatformPositions = (data) => {
        const _rrTabPosition = data?.map((item, index) => ({
            value: item.value,
            position: index + 1
        }));
        // localStorage.setItem("RRTabPosition", JSON.stringify(_rrTabPosition));
        let tabsPayload = {
            "type": 'rr_plateform',
            "key": 'ComprehensivePosition',
            "ComprehensivePosition": _rrTabPosition,
          }
    ...
    const loadAndRearrangePlatformPositions = (data) => {
        ...
        // const storedPosition = JSON.parse(localStorage.getItem("RRTabPosition"));
    ...
    useEffect(() => {
        if (platform) {
        let _rrTabPosition = tab_headers.map((item, index) => ({
            value: item,
            position: index + 1
        }));
        ...
        if (!storedPosition || storedPosition.length === 0) {
            savePlatformPositions(_rrTabPosition ?? []);
        } else {
            _rrTabPosition = loadAndRearrangePlatformPositions(_rrTabPosition ?? []);
        }
        setRRTabPosition(_rrTabPosition);
        setSelectedTabName(_rrTabPosition?.[0]?.value)
    }
    }, [platform]);
    const handleDragEnd = (event) => {
        ...
        if (over && active?.id !== over?.id) {
            setRRTabPosition((prevPosition) => {
                ...
                const _rrTabPosition = newdata.map((item, index) => ({
                    value: item.value,
                    position: index + 1
                }));
                // localStorage.setItem("RRTabPosition", JSON.stringify(_rrTabPosition));
                let tabsPayload = {
                  ...
                  saveOSAPlateform(tabsPayload);
                const activeIndex = _rrTabPosition.findIndex((item) => item.value === selectedTabName);
    ...
    useEffect(() => {
        const loadPlatformData = async () => {
          try {
            ...
            // }
            if (platformSubCat?.rr_plateform?.ComprehensivePosition) {
                setPlatform(platformSubCat.rr_plateform);
              }
          } catch (error) {
    ...
    const handleTabSelect = (index) => {
        ...
        setSelectedTabName(rrTabPosition?.[index]?.value);
    };
    return (
        <>
                                    ...
                                    <div className='tabListWrap'>
                                        <div className='tabList'>
                                            <DndContext sensors={sensors} onDragEnd={handleDragEnd} >
                                                <SortableContext items={rrTabPosition.map((item) => item.value)} >
                                                    {rrTabPosition?.length && rrTabPosition?.map((item) => (
                                                        <Tab key={item.value}>
                                                            <SortableTab
                                                                key={item.value}
                                                                id={item.value}
