...
const BreakdownGraph = ({ drawerInfo,reportTableData = [], perviousReportTableData = [], loading }) => {
  const {
    platformColor,
    percentageIcon,
    defaultOption
  ...
  const memoizedOptionGraph = useMemo(() => {
    ...
    if (series.length > 0) {
      ...
      return {
        ...
        title: {
          show: false,
          text: `Comprehensive Breakdown Graphical Analysis of Selected ${drawerInfo?.column}`,
          // subtext:` - ${_lables}`,
          top: 0,
          textStyle: {
            fontSize: 14,
