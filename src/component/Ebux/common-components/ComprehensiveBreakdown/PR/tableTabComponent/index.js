...
const TableTabComponent = ({
    ...
    const headerkey = { "price_variation": "Average Discount", "price_rp": "MRP", "price_sp": "Selling Price" };
    const dataKey = dataHeader.toLowerCase();
    const sortedData = useMemo(() => {
        if (sortConfig.key) {
            return [...reportTableData].sort((a, b) => {
                const _sortkey = sortConfig?.key.split(".");
                a = sortConfig.key == 'item' ? (a?.[sortConfig.key]?.lable) : sortConfig.key == 'sku_name' ? (a?.['item']?.sku_name) : (a?.[_sortkey[0]]?.[_sortkey[1]]);
                b = sortConfig.key == 'item' ? (b?.[sortConfig.key]?.lable) : sortConfig.key == 'sku_name' ? (b?.['item']?.sku_name) : (b?.[_sortkey[0]]?.[_sortkey[1]]);
    ...
    const handleDownload = () => {
        ...
        const title = `${dataHeader} Promotions Report`;
        ...
        sortedData?.forEach((data) => {
            ...
            pfkey.map((pf) => (
                Object.keys(headerkey).map((hk) => {
                    ...
                    _data[`${pf}_${headerkey[hk]}`] = data?.[pf]?.[hk] === undefined ? "-" : `${hk != "price_variation" ? "Rs." : ""} ${data?.[pf]?.[hk]}${hk == "price_variation" ? percentageIcon : ''}`;
                })
            ))
    ...
    const getDataAVG = (key) => {
        ...
        return (isNaN(result)) ? '-' : <>{_key?.[1] != "price_variation" && (<LuIndianRupee />)} {result?.toFixed(2)} {_key[1] == "price_variation" ? percentageIcon : ''}</>;
    ...
    const handleClickOutside = (event) => {
        ...
        !document.getElementById("tooltip-container")?.contains(event.target)) {
            setActiveTooltip(null); // Close the tooltip
        }
    };
            ...
            <tr key={`${data?.item?.value}-${index}`} className={`breakdownRow ${isSelectAllChecked || selectedTableRows?.[dataKey]?.some((item) => item?.value === data?.item?.value) ? "selectedRow" : ""}`}>
                ...
                <td>
                    <div className='flex flex-row gap-2 justify-between'>
                        <p className="flex-grow">{data?.item?.lable || "-"} </p>
                        <button className='breakdownChartIcon' type="button" onClick={() => openDrawer(dataKey, data?.item?.value, data?.item?.lable, data?.item?.pdp_image_url, data?.item?.sku_name)}><img src="/assets/images/graphIcon.svg" width={18} height={18} /></button>
                    </div>
                </td>

                {dataHeader == "SKU" && (
                    <td className="skuProductCol overflow-visible z-[1]">
                        <div className="relative z-[99]">
                            <div className="flex flex-row gap-2 justify-between skuProductName">
                                <div className="relative w-[250px]">
                                    {data?.item?.sku_name
                                        ? data?.item?.sku_name.length > 35
                                            ? `${data?.item?.sku_name.slice(0, 35)}`
                                            : data?.item?.sku_name
                                        : "-"}
                                    <span
                                        onClick={(e) => {e.stopPropagation(); toggleTooltip(data?.item?.value)}} // Pass a unique ID or index
                ...
                )}
                {
                    pfkey.map((pf, i) => (
                        ...
                        (<td key={i + index}>
                            {data?.[pf]?.[hk] === undefined ? "-" : <><div className='flex flex-row gap-0.5 justify-center items-center'>{hk != "price_variation" && (<LuIndianRupee />)} {data?.[pf]?.[hk]?.toFixed(2)}{hk == "price_variation" ? percentageIcon : ''}</div></>}
                        </td>)
                        )
                    ))
                }
    ...
    return (
        <>
            <div className="tblHeadCatWrap" ref={containerRef}>
                ...
                <table className="tblOuter">
                    <thead>
                        <tr>
                            ...
                            {dataHeader == "SKU" &&
                                (<th className="skuProductCol" onClick={() => requestSort('sku_name')}><div className="sortingCol w-full py-2 skuProductName" >Product Name
                                    <SortingButton sortType={sortConfig.key === 'sku_name' ? sortConfig.direction : ''} />
                                </div></th>)
                            }
                            {
                                pfkey.map((pf, i) => (
