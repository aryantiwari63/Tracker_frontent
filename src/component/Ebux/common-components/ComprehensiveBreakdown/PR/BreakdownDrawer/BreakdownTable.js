...
const BreakDownTable = ({ drawerInfo,reportTableData }) => {
  ...
  const renderTableRows = () => {
    return sortedData?.map((data,_index) => (
      <tr key={data?.item?.value??data?.date??_index} >
        ...
        <td>
          <div className='flex flex-row gap-2 justify-between whitespace-nowrap'>
            {data?.date || "-"}
          </div>
        </td>
        {
          pfkey.map((pf, i) => (
            ...
            (<td key={i + index}>              
            {data?.[pf]?.[hk] === undefined ? "-" : <div className="flex items-center justify-center">{hk != "price_variation" && (<LuIndianRupee />)} {data?.[pf]?.[hk]?.toFixed(2)}{hk == "price_variation" ? percentageIcon : ''}</div>}
  ...
  const header = "Discount Breakdown";
  return (
    <>
