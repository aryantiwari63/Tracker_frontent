...
const BrandFilter = ({ expanded, onToggle, globalSearch = "" }) => {
    const {
        kpi,
        filters,
    ...
    const heading = kpi === "SOS" ? "Keyword Brand" : "Brand";
    ...
    const handleSelectAllUpdate = (options, allSelected) => {
        ...
        if (allSelected) {
            newItems = (localSelectedBrands || []).filter(
                (b) => !options.some((opt) => String(opt.value) === String(b.value))
            );
        } else {
            const toAdd = options.filter(opt => !localSelectedBrands.some(sel => String(sel.value) === String(opt.value)));
            newItems = [...localSelectedBrands, ...toAdd];
        }
        setSelectedFiltersWidget(prevFilters => ({
            ...prevFilters,
            selectedBrand: newItems ?? [],
        }));
    };

    const sortedBrands = useMemo(() => {
        const list = Array.isArray(filters?.brand) ? filters.brand : [];
        const searchInput = (brandSearch || globalSearch || "").trim();
        ...
        const selectedValues = (localSelectedBrands || []).map((b) => String(b.value));
    ...
    }, [filters?.brand, globalSearch, brandSearch, localSelectedBrands]);
    ...
    const allVisibleSelected = sortedBrands.length > 0 &&
        sortedBrands.every(opt => localSelectedBrands.some(sel => String(sel.value) === String(opt.value)));
    const someVisibleSelected = !allVisibleSelected &&
        sortedBrands.some(opt => localSelectedBrands.some(sel => String(sel.value) === String(opt.value)));

    const showNoResults = sortedBrands.length === 0 && (brandSearch?.trim() || globalSearch?.trim());

    return (
        <div className="pb-4 mb-4 border-b border-gray-100 last:border-0 last:pb-0 last:mb-0">
            <div className="flex items-center justify-between cursor-pointer group" onClick={() => onToggle("brand")}>
            ...
            <div className={`transition-all duration-300 ${expanded ? "max-h-[500px] mt-4 opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}>
                ...
                {showNoResults && (
                    <div className="text-sm text-gray-500 py-4 text-center bg-gray-50 rounded-lg">
                        No {heading.toLowerCase()}s found for <span className="font-semibold">{brandSearch || globalSearch}</span>
                    </div>
                )}
            </div>
        </div>
