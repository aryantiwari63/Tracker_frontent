      ...
      <div className="p-1 border-l ">
        <span className="my-1 ml-1 px-2 rounded-sm bg-[#cce6fd] text-[#0081F7]  ">
          ...
          {(FILTERACTION.TAG === obj.action
            ? obj?.label
            : chipLabelNameFixObj[obj?.key] || obj.key)?.replace(':|@|:', '-')}
        </span>
      </div>
      <>
        {mapObj?.sign && (
          <div className="p-1 border-l flex items-center">
            {mapObj?.sign === "between" || mapObj?.sign === "isn't between" ? (
              <span title={mapObj?.sign} className="font-normal block max-w-[60px] truncate">{mapObj?.sign}</span>
            ) : (
              <span title={mapObj?.sign} className="px-1 border rounded-sm block max-w-[60px] truncate">{mapObj?.sign}</span>
            )}
          </div>
        )}
        {mapObj?.value && (
          <div className="p-1 border-l">
            <span title={mapObj?.value} className="block my-1 rounded-sm px-2 bg-[#cce6fd] text-[#0081F7] max-w-[60px] truncate">
              {mapObj?.value}
            </span>
          </div>
        )}
