...
const FilterMenu = ({
  ...
  function checkAllSelected(key, children, selectedListObj) {
    ...
    return children.every((child) => {
      ...
      return selectedListObj?.[childKey] ?? false;
    });
  }
      ...
      {showList &&
        obj?.children?.map((item, index) => {
          return Array.isArray(item?.children) ? (
            <div key={`nested-${index}`} className="pl-4">
              <FilterMenu
                obj={item}
                chips={chips}
                ...
                isAllSelected={isAllSelected||(checkAllSelected(obj?.key, item?.children, selectedListObj))}
              />
            </div>
          ) : (
            <div
