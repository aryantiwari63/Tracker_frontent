...
function SortableItem({ id, item }) {
  ...
  const getPlatfomValue = (_platform = "") => {
    if (_platform in averagePercentageData) {
      const val =
        averagePercentageData?.[_platform]?.currentData?.toFixed(2) ?? null;
      return val || val == 0 ? `${decimalValueManager(val) + percentageIcon}` : "...";
    } else {
      return "...";
    }
  };
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes}>
      <div
        ...
        onClick={() => handlePlatformTabClickNew(item)}
        onDoubleClick={() => handlePlatformTabClickNew(item, true)}
...
const PlatformTabsComponent1 = ({ type }) => {
  const {
  ...
  const isDs3 = activeClientProject?.uiType === "ds3";
  if (isDs3) return null;
...
export default PlatformTabsComponent1;
