...
import KeywordCloud from './common-components/KeywordCloud';
...
const RatingReviews = ({ type = 1 }) => {
    ...
    const loadPlatformData = async () => {
        ...
        try {
            ...
            if (response?.userRecord?.tabs_plateform?.RRPosition?.length) {
                ...
                //       "position": 3
                //     },{
                //        "value": "PlatformChart",   "position": 4
                //     },{
                //         "value": "KeywordCloud",
                //         "position": 5
                //     }
                    ...
                    <SortableContext items={(Array.isArray(platform) ? platform : []).map((item) => item.value)} >
                        <div className="grid grid-cols-2 gap-4 w-full">
                            {(Array.isArray(platform) ? platform : []).map((item) => {
                                ...
                                if (type == 1 && ["OsaByBrand", "OosDaysOverview", "OsaPerformanceOverview", "DetailsAnalysisone", "CompDetailsAnalysisone", "ReviewDetails", "DetailsAnalysis", "DayonDayTable", "PlatformDistribution", "TableDayOnDay", "DayonDayTable", "PlatformDistribution"]?.indexOf(item.value) > -1) {
                                    return <></>;
                                } else if (type == 2 && ((["ComprehensiveBreakdown", "DetailsAnalysisone", "CompDetailsAnalysisone", "ReviewDetails", "DetailsAnalysis", "KeywordCloud"]?.indexOf(item.value) > -1) || notAllow)) {
                                    return <></>;
                                }
                                return (
                                    <div
