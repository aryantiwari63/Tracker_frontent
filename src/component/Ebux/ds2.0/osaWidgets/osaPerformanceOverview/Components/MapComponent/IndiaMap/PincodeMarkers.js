...
export default function PincodeMarkers({activefromTable,is_dark_store,metrics,darkStoreLocationData, pincodeList,updateLocationSteps,onPincodeClick }) {
  const [activePincode, setActivePincode] = useState(null);

  return (
    <>
      {darkStoreLocationData?.data?.map((item) => {
        const city=pincodeList?.find((pincode) => pincode.name == item.pincode);    
        if (!city) {
          return null;
        }
        ...
        const getTooltip=(stateName)=>{
          if(activePincode == stateName||activefromTable == stateName){
        ...
        return (
          <Marker
            ...
            eventHandlers={{
              ...
              //   setActivePincode((prev) => (prev === city.name ? null : city.name));
              // },
              click: () =>{if(is_dark_store){onPincodeClick(item.pincode)}} ,
              mouseover: () =>setActivePincode(item.pincode),
              mouseout:()=>setActivePincode(null)
            }}
          >
