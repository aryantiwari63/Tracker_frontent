...
// import ComprehensiveBreakdownComponent from './common-components/ComprehensiveBreakdown/CS';
import BreakdownComponent from './common-components/ComprehensiveBreakdown/CS/breakdown';
...
const ContentScore = ({ type = 1 }) => {
    ...
    const savePlatformPositions = async (data) => {
        ...
        //     position: index + 1
        // }));
        // localStorage.setItem("CSPosition", JSON.stringify(platformPosition));
    ...
    //     const storedPlatformPosition = JSON.parse(localStorage.getItem("CSPosition"));
    ...
    const loadPlatformData = async () => {
        try {
            ...
            // console.log('=====',response.userRecord.tabs_plateform.CSPosition)
            if (response?.userRecord?.tabs_plateform?.CSPosition?.length) {
                setPlatform(response.userRecord.tabs_plateform.CSPosition);
    ...
    return (
        <>
            {type == 1 ? <PlatformTabsComponent type={"cs_plateform"} /> : <PlatformTabsComponent1 type={"cs_plateform"} />}
