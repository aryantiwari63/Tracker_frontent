...
import BreakdownComponent from './common-components/ComprehensiveBreakdown/SOS/breakdown';
const SosWidgets = React.lazy(() => import('./ds2.0/sosWidgets/index'));
...
function SortableItem({ id, platformSubCat }) {
    ...
    const getComponent = (ComponentName = "", listeners, attributes) => {
        ...
        switch (ComponentName) {
            ...
            case "Focuscharts":
                return <FocusChartsComponent chartTitle='Keyword Analysis' chartType='Keyword' chartLable={activeClientProject?.useWeightedSOS ? 'Wt. SOS' : 'SOS'} listeners={listeners} attributes={attributes} platformSubCat={platformSubCat} />;
            ...
            case "SosWidgets":
                return <React.Suspense fallback={<div>Loading...</div>}><SosWidgets listeners={listeners} attributes={attributes} /></React.Suspense>;
            case "DayonDayTable":
...
const SOS = ({ type = 1 }) => {
    ...
    const loadPlatformData = async () => {
        try {
            ...
            if (response?.userRecord?.tabs_plateform?.SOSPosition?.length) {
                setPlatform(response.userRecord.tabs_plateform.SOSPosition);
                setPlatformSubCat(response.userRecord)
            } else {
                // If no data from API, initialize with default data
                const defaultPlatform = [
                    { value: "SosWidgets", position: 0 },
                    { value: "PlatformChart", position: 1 },
                    { value: "Focuscharts", position: 2 },
                    { value: "ComprehensiveBreakdown", position: 3 },
                ];
                let tabsPayload = {
                    "SOSPosition": defaultPlatform,
                }
    ...
    useEffect(() => {
        handleChange(null, type == 1 ? "executive_summary" : filtersDarkStore?.tab_type)
    ...
    return (
        <>
            {type == 1 ? <PlatformTabsComponent type={"sos_plateform"} /> : <PlatformTabsComponent1 type={"sos_plateform"} />}
                    ...
                    <SortableContext items={(Array.isArray(platform) ? platform : []).map((item) => item.value)} >
                        <div className="grid grid-cols-2 gap-4 w-full">
                            {(Array.isArray(platform) ? platform : []).map((item) => {
                                ...
                                if (type == 1 && ["SosWidgets", "OsaByBrand", "OosDaysOverview", "OsaPerformanceOverview", "TableDayOnDay", "DayonDayTable", "PlatformDistribution"]?.indexOf(item.value) > -1) {
                                    return <></>;
